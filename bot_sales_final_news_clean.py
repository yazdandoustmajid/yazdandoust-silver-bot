...
async def get_board_gold_snapshot():
    """Get board gold values with Taban Gohar -> TGJU fallback."""
    try:
    ...
    gold = None
    try:
        gold_html = await asyncio.to_thread(
            http_get,
            TGJU_GOLD_URL,
            30,
        )
        gold = parse_tgju_current_price(gold_html, "gold")
    except Exception as error:
        log.warning("TGJU BOARD GOLD FAILED | %s", error)

    try:
        ounce = await get_gold_ounce()
    except Exception as error:
        log.warning("TGJU BOARD GOLD OUNCE FAILED | %s", error)
        ounce = None

    if gold is None or ounce is None:
        return None
    return {"gold_18": int(gold), "gold_ounce": float(ounce)}


async def get_board_silver_dollar_snapshot():
    """Live global silver ounce + Tehran free-market dollar rate from TGJU,
    independent of whatever the source channel posts. Mirrors
    get_board_gold_snapshot() above so every board reflects live market
    data for these two fields as well, not just the channel's own cadence."""
    ounce = None
    tehran = None

    try:
        silver_item = await asyncio.to_thread(
            fetch_tgju_market_item,
            "silver_ounce",
            MARKET_PULSE_TGJU_SYMBOLS["silver_ounce"],
        )
        if silver_item and silver_item.get("value") is not None:
            ounce = float(silver_item["value"])
    except Exception as error:
        log.warning("TGJU BOARD SILVER OUNCE FAILED | %s", error)

    try:
        dollar_item = await asyncio.to_thread(
            fetch_tgju_market_item,
            "dollar",
            MARKET_PULSE_TGJU_SYMBOLS["dollar"],
        )
        if dollar_item and dollar_item.get("value") is not None:
            tehran = int(round(dollar_item["value"]))
    except Exception as error:
        log.warning("TGJU BOARD DOLLAR FAILED | %s", error)

    if ounce is None and tehran is None:
        return None
    return {"ounce": ounce, "tehran": tehran}
...
def calculated_shot_levels(
    chart_rows,
    current
):
    """
    حمایت/مقاومت روزانه ساچمه ۹۹۵ را از داده واقعی همان روز
    محاسبه می‌کند.

    مبنا:
    - Pivot Classic بر اساس سقف، کف و قیمت پایانی روز
    - S1 / S2 / R1 / R2
    - کف‌ها و سقف‌های محلی برای کنترل منطقی سطوح
    """
    values = [
        float(row["value"])
    ...
    # سطوح خام Pivot را به نزدیک‌ترین هزار/پانصد گرد نمی‌کنیم؛
    # عدد واقعی محاسبه‌شده حفظ می‌شود تا دقت تحلیل از بین نرود.
    levels = {
        "pivot": pivot,
        "support": s1 if s1 < current else None,
        "support_2": s2 if s2 < current else None,
        "resistance": r1 if r1 > current else None,
        "resistance_2": r2 if r2 > current else None,
    }

    # اگر قیمت دقیقاً روی R1/S1 باشد، سطح بعدی را به‌عنوان سطح فعال نگه می‌داریم.
    if levels["support"] is None and s1 <= current:
        levels["support"] = s1
    if levels["support_2"] is None and s2 < (levels["support"] or current):
        levels["support_2"] = s2
    if levels["resistance"] is None and r1 >= current:
        levels["resistance"] = r1
    if levels["resistance_2"] is None and r2 > (levels["resistance"] or current):
        levels["resistance_2"] = r2

    # محدوده‌ها فقط برای خواناتر شدن تصویر هستند؛ عدد مرکزی همان Pivot واقعی است.
    zone_width = max(
        abs(high - low) * 0.06,
        abs(current) * 0.00035
    )
    ...
    def zone(value):
        if value is None:
            return None
        return (
            max(0.0, value - zone_width),
            value + zone_width
        )

    levels["support_zone"] = zone(levels["support"])
    levels["support_zone_2"] = zone(levels["support_2"])
    levels["resistance_zone"] = zone(levels["resistance"])
    levels["resistance_zone_2"] = zone(levels["resistance_2"])

    return levels


def daily_caption_fallback(
    snapshot
):
    """
    کپشن خودمانی و متغیر برای پست تصویری روزانه.
    در صورت در دسترس نبودن AI نیز هر روز متن ثابت منتشر نمی‌شود.
    """
    shot = snapshot.get("shot_995", {})
    silver = snapshot.get("silver", {})

    shot_change = shot.get("daily_change_percent")
    silver_change = silver.get("daily_change_percent")

    score = 0.0
    if shot_change is not None:
        score += float(shot_change)
    if silver_change is not None:
        score += float(silver_change)

    if score <= -1.5:
        variants = [
            "امروز نقره حسابی تحت فشار بود 😳\n\nساچمه ۹۹۵ تا کف روز پایین رفت و بعد دوباره برگشت. حالا مهم‌ترین سؤال اینه که این برگشت واقعی بوده یا فقط یه نفس قبل از ادامه ریزش؟\n\nحمایت‌ها و مقاومت‌های امروز رو دقیق حساب کردم؛ محدوده‌ای که الان قیمت نزدیکشه رو حتماً ببینید 👇",
            "امروز بازار نقره اصلاً شوخی نداشت! 🔴\n\nفشار فروش باعث شد ساچمه ۹۹۵ چند پله پایین بیاد، ولی آخر روز دوباره برگشت.\n\nاگر ساچمه دارید یا قصد خرید دارید، به نظرم قبل از هر تصمیم یه نگاه به محدوده‌های حمایت و مقاومت امروز بندازید؛ نقطه مهم بازار داخل تحلیله 👇",
            "یه روز پرنوسان برای نقره داشتیم 👀\n\nقیمت پایین اومد و بعد بخشی از ریزش رو پس گرفت؛ ولی هنوز نمی‌شه گفت خطر فشار فروش تموم شده.\n\nتوی تصویر دقیقاً مشخص کردم کجا خریدارها باید وارد عمل بشن و کجا ممکنه دوباره فروشنده‌ها فعال بشن 👇",
        ]
    elif score >= 1.5:
        variants = [
            "امروز نقره یه حرکت خوب زد 🚀\n\nساچمه ۹۹۵ دوباره خودش رو بالاتر کشید و حالا رسیده به محدوده‌ای که شکستنش می‌تونه مهم باشه.\n\nحمایت‌ها و مقاومت‌های امروز رو دقیق حساب کردم؛ ببینید قیمت برای ادامه مسیر با چه عددهایی روبه‌روئه 👇",
            "امروز سمت خریدارها قوی‌تر بود 🟢\n\nنقره حرکت رو به بالا داشت، اما هنوز یه مقاومت مهم جلوی راهشه.\n\nتوی تحلیل امروز مشخص کردم کجا ادامه رشد تأیید می‌شه و کجا احتمال برگشت بیشتره. این قسمت رو حتماً ببینید 👇",
            "نقره امروز یه نشونه مثبت داد 👀\n\nساچمه ۹۹۵ بالاتر رفت و حالا بازار دقیقاً نزدیک محدوده‌ایه که می‌تونه مسیر حرکت بعدی رو مشخص کنه.\n\nحمایت و مقاومت‌ها رو بر اساس داده‌های امروز حساب کردم؛ داخل تصویر ببینید 👇",
        ]
    else:
        variants = [
            "امروز نقره یه مقدار رفت‌وبرگشت داشت و آخرش تقریباً سر جای اولش ایستاد 👀\n\nولی همین‌جا یه نکته مهم داریم؛ قیمت بین چند محدوده حساس قرار گرفته.\n\nحمایت و مقاومت‌های دقیق امروز رو داخل تصویر گذاشتم. ببینید کدوم عدد می‌تونه مسیر بعدی رو مشخص کنه 👇",
            "امروز بازار بیشتر درگیر نوسان بود تا یک حرکت مشخص.\n\nساچمه ۹۹۵ پایین رفت و دوباره برگشت؛ برای همین الان حمایت و مقاومت از خودِ درصد تغییر مهم‌تره.\n\nمحدوده‌های مهم رو دقیق حساب کردم، داخل تحلیل ببینید 👇",
            "حرکت امروز نقره خیلی واضح نبود؛ نه صعود جدی، نه ریزش جدی.\n\nاما قیمت به چند محدوده حساس نزدیکه و شکست یکی از اون‌ها می‌تونه جهت حرکت بعدی رو مشخص کنه.\n\nتحلیل امروز رو ببینید، مخصوصاً بخش حمایت و مقاومت 👇",
        ]

    # انتخاب روزانه متفاوت، بدون نیاز به ذخیره state جدید.
    index = iran_now().toordinal() % len(variants)
    return variants[index] + "\n\n📊 تحلیل روزانه نقره | یزدان‌دوست\n📲 @yazdandoustsilver"


async def ai_daily_caption(
    snapshot
):
    """
    کپشن کوتاه، خودمانی و متناسب با وضعیت همان روز.
    """
    if not OPENAI_API_KEY:
        return daily_caption_fallback(snapshot)

    shot = snapshot.get("shot_995", {})
    silver = snapshot.get("silver", {})
    levels = shot.get("levels", {})

    prompt = f"""
برای کانال تلگرام «یزدان‌دوست» یک کپشن کوتاه و خودمانی برای تحلیل روزانه نقره بنویس.

داده‌های واقعی امروز:
{json.dumps(snapshot, ensure_ascii=False, indent=2, default=str)}

کپشن باید طوری باشد که انگار صاحب کانال خودش نوشته؛ تبلیغاتی و رسمی نباشد.
هدف اصلی این است که مخاطب برای دیدن تصویر تحلیل کنجکاو شود.

قوانین:
- از اتفاق واقعی امروز شروع کن؛ اگر بازار خیلی ریزشی است هیجان هشدار بیشتر، اگر خیلی صعودی است هیجان مثبت بیشتر.
- اگر بازار خنثی است، روی محدوده حساس و انتظار برای شکست آن تمرکز کن.
- نتیجه کامل تحلیل را در کپشن لو نده؛ مخاطب را برای باز کردن تصویر نگه دار.
- حمایت و مقاومت را در کپشن عددسازی نکن؛ اعداد دقیق داخل تصویر هستند.
- بین 70 تا 130 کلمه.
- خودمانی، روان و طبیعی؛ مثل تایپ صاحب یک کانال واقعی.
- حداکثر 4 پاراگراف کوتاه.
- ایموجی کم اما به‌جا.
- در پایان دقیقاً این دو خط را بیاور:
📊 تحلیل روزانه نقره | یزدان‌دوست
📲 @yazdandoustsilver
- هیچ هشتگ، جدول یا عبارت «این توصیه خرید یا فروش نیست» اضافه نکن.
"""
    ...
    try:
        ...
        response = client.responses.create(
            model=OPENAI_MODEL,
            instructions=(
                "تو نویسنده کپشن یک کانال تخصصی نقره هستی. "
                "لحن باید طبیعی، خودمانی، کوتاه و کنجکاوی‌برانگیز باشد."
            ),
            input=prompt
        )

        result = clean_ai_output(response.output_text)
        if result and len(result) <= 1200:
            return result

    except Exception as error:
        log.warning(
            "AI DAILY CAPTION FAILED: %s",
            error
        )
...
def create_daily_silver_image(
    ...
    """
    داشبورد حرفه‌ای تحلیل روزانه نقره یزدان‌دوست.
    تمام اعداد از snapshot واقعی همان روز می‌آیند.
    """
    raw_rows = snapshot.get("chart_rows", [])
    ...
    width = 1400
    height = 1750
    image = Image.new("RGB", (width, height), (7, 15, 14))
    ...
    panel2 = (13, 31, 29)
    border = (55, 84, 74)
    text_main = (242, 245, 242)
    text_soft = (172, 190, 182)
    muted = (125, 148, 138)
    green = BRAND_UP_COLOR
    red = BRAND_DOWN_COLOR
    white = (236, 241, 238)
    ...
    if YAZDANDOUST_LOGO.exists():
        try:
            logo = Image.open(YAZDANDOUST_LOGO).convert("RGBA")
            max_w, max_h = 230, 150
    ...
    draw_rtl(
        draw,
        (width - 70, 201),
        f"گزارش روزانه بازار | {date_text}",
        small_font,
        text_soft
    )

    current = snapshot.get("shot_995", {}).get("current")
    change = snapshot.get("shot_995", {}).get("daily_change_percent")
    stats = daily_stats(raw_rows)

    if change is None:
        change = stats.get("change_percent")

    current_text = (
        format_level(current, 0)
    ...
    draw.text(
        ...
        font=value_font,
        fill=(
            green if (change is None or change >= 0)
            else red
        )
    )

    # ---------- chart ----------
    chart_x1, chart_y1 = 500, 270
    chart_x2, chart_y2 = width - 45, 650
    ...
    draw_rtl(
        draw,
        (chart_x2 - 20, chart_y1 + 25),
        "نمودار حرکت روزانه ساچمه ۹۹۵",
        section_font,
        text_main
    )
    ...
    pad = max((high - low) * 0.14, 1)
    plot_low = low - pad
    plot_high = high + pad
    px1, py1 = chart_x1 + 65, chart_y1 + 90
    px2, py2 = chart_x2 - 25, chart_y2 - 55
    ...
    for i in range(5):
        ...
        tick = plot_high - (plot_high - plot_low) * i / 4
        draw.text(
            (px1 - 8, y - 10),
            format_level(tick, 0),
            font=tiny_font,
    ...
    for i, row in enumerate(raw_rows):
        x = px1 + (px2 - px1) * i / max(len(raw_rows) - 1, 1)
        y = py2 - (
            (float(row["value"]) - plot_low)
            / (plot_high - plot_low)
        ) * (py2 - py1)
        points.append((int(x), int(y)))

    line_fill = green if values[-1] >= values[0] else red
    ...
    draw.text(
        (max(px1, hx - 55), max(py1, hy - 40)),
        f"سقف {format_level(high, 0)}",
        font=tiny_font,
        fill=green
    )
    draw.text(
        (max(px1, lx - 55), min(py2 - 5, ly + 14)),
    ...
    gap = 18
    stat_items = [
        ("شروع", stats.get("start")),
        ("کف روز", stats.get("low")),
        ("سقف روز", stats.get("high")),
        ("پایان", stats.get("end")),
        ("میانگین", (
    ...
    for i, (label, value) in enumerate(stat_items):
        ...
        draw_rtl(
            draw,
            (x2 - 16, stat_y + 67),
            format_level(value, 0) if value is not None else "—",
            medium_font,
            white
        )

    # ---------- calculated support / resistance ----------
    levels = snapshot.get("shot_995", {}).get("levels", {})
    pivot = levels.get("pivot")
    s1 = levels.get("support")
    s2 = levels.get("support_2")
    r1 = levels.get("resistance")
    r2 = levels.get("resistance_2")

    draw.rounded_rectangle(
        (45, 835, width - 45, 1110),
        radius=24,
        fill=panel,
        outline=border,
        width=2
    ...
    draw_rtl(
        draw,
        (width - 75, 865),
        "حمایت و مقاومت‌های محاسبه‌شده",
        section_font,
        text_main
    )

    # سه نقطه مرکزی: حمایت‌ها / پیوت / مقاومت‌ها
    items = [
        ("حمایت ۲", s2, green, 85),
        ("حمایت ۱", s1, green, 320),
        ("پیوت", pivot, white, 555),
        ("مقاومت ۱", r1, red, 790),
        ("مقاومت ۲", r2, red, 1025),
    ]

    for label, value, color, x in items:
        draw_rtl(
            draw,
            (x + 185, 925),
            label,
    ...
    line_y = 1045
    draw.line(
        (90, line_y, width - 90, line_y),
        fill=(82, 103, 96),
        width=3
    )
    for _, value, color, x in items:
        if value is None:
            continue
        draw.ellipse(
            (x + 177, line_y - 8, x + 193, line_y + 8),
            fill=color
        )

    # ---------- market drivers ----------
    draw.rounded_rectangle(
        (45, 1140, width - 45, 1390),
        radius=24,
        fill=panel2,
        outline=border,
        width=2
    ...
    drivers = [
        (
            "انس نقره",
            silver.get("current"),
            silver.get("daily_change_percent"),
            2
        ),
        (
            "طلا ۱۸ عیار",
            gold.get("current"),
            gold.get("daily_change_percent"),
            2
        ),
        (
            "دلار (شاخص جهت دلار)",
            usd.get("current_toman"),
            usd.get("daily_change_percent"),
            0
        ),
        (
            "DXY",
            dxy.get("current"),
            dxy.get("daily_change_percent"),
            3
        ),
    ]

    for i, (label, value, pct, decimals) in enumerate(drivers):
        x1 = 70 + i * 330
        x2 = x1 + 300
        draw.rounded_rectangle(
            (x1, 1215, x2, 1360),
    ...
    if change is not None:
        score += float(change)
    if silver.get("daily_change_percent") is not None:
        score += float(silver["daily_change_percent"])

    if score <= -1.5:
        status = "نزولی"
        status_color = red
        conclusion = "فشار فروش بیشتر است؛ حمایت‌ها را زیر نظر داشته باشید."
    elif score >= 1.5:
        status = "صعودی"
        status_color = green
        conclusion = "قدرت خریداران بیشتر است؛ شکست مقاومت‌ها مهم است."
    else:
        status = "نوسانی"
        status_color = (220, 190, 90)
        conclusion = "بازار هنوز جهت قطعی ندارد؛ شکست محدوده‌ها تعیین‌کننده است."

    draw.rounded_rectangle(
        (45, 1415, width - 45, 1555),
        radius=22,
        fill=panel,
        outline=border,
        width=2
    ...
    draw_rtl(
        draw,
        (width - 70, 1622),
        "یزدان‌دوست سیلور | تحلیل بازار نقره",
        small_font,
        text_soft
    )
    draw_rtl(
        draw,
        (width - 70, 1660),
        "حمایت و مقاومت بر اساس محاسبات قیمت روز",
        tiny_font,
        muted
    )
...
async def send_daily_silver_analysis(
    ...
    if shot_raw is None:
        log.error(
            "DAILY SILVER ANALYSIS SKIPPED | shot_995 unavailable"
        )
        return None
    ...
    snapshot.setdefault("shot_995", {})
    snapshot["shot_995"]["current"] = shot_current
    snapshot["shot_995"]["daily_change_percent"] = (
    ...
    # حمایت/مقاومت ساچمه ۹۹۵ مستقیماً از سقف، کف و پایان روز
    # با Pivot Classic محاسبه می‌شود؛ نه با حدس یا عدد ثابت.
    snapshot["shot_995"]["levels"] = calculated_shot_levels(
        chart_rows,
        shot_current
    )

    snapshot["chart_rows"] = chart_rows

    image = create_daily_silver_image(snapshot)

    caption = await ai_daily_caption(snapshot)

    if len(caption) > NEWS_MEDIA_CAPTION_LIMIT:
        caption = (
            caption[:NEWS_MEDIA_CAPTION_LIMIT - 35].rstrip()
            + "\n\n📲 @yazdandoustsilver"
        )
    ...
    try:
        ...
        if daily_message:
            await send_rubika_media(daily_message)
    except Exception as error:
        log.warning(
...
19. متن خبر باید از همان جمله اول اصل اتفاق را روشن و مستقیم بیان کند؛ برای انتشار عمومی تیتر یا هدر جداگانه لازم نیست.
...
def make_news_caption(
    ...
    # پست عمومی عمداً بدون عنوان/هدر منتشر می‌شود.
    # عنوان خبر فقط برای پردازش داخلی، جلوگیری از تکرار و فیلترها نگه داشته می‌شود.
    # متن باید از همان جمله اول، اصل خبر را ساده و حرفه‌ای منتقل کند.
    parts = []
...
async def restore_automatic_prices_after_manual_expiry(client, target, state):
    ...
    try:
        ...
        # The automatic board should use the live public sources, with only
        # any still-active manual overrides applied on top.
...
def hourly_key(name):
    jy, jm, jd = iran_date_parts()
    now = iran_now()
    return f"{jy:04d}-{jm:02d}-{jd:02d}_{now.hour:02d}_{name}"


def should_send_hourly(state, name):
    return state.get(hourly_key(name)) != True


def mark_hourly_sent(state, name):
    state[hourly_key(name)] = True
    ...
    while (
        ...
        try:
            ...
            if source_changed:
                ...
                if since_last_post < LIVE_POST_MIN_INTERVAL_SECONDS:
                    log.info(
...
async def _main_locked():
    ...
    try:
        ...
        if (
            ...
            # The board is intentionally not a ticker. A new channel board is
            # published only when 995 shot or 999.9 ingot changes. Dollar and
            # ounce can move freely and will be reflected on the next product
            # publication.
            product_trigger = (
                state.get("shot_995") is None
                or state.get("nader_9999") is None
            ...
            if not isinstance(pending, dict):
                ...
                if product_trigger and current_signature != previous_signature:
        ...
        # =================================================
        # HOURLY LIVE MARKET REPORT
        # =================================================
        # Runs every hour on its own, independent of the source
        # (Taghizadegan) channel: it posts whether or not that channel
        # posted an update this hour. If the channel also posts an update,
        # this hourly report still runs on its own schedule in addition to
        # it (they are independent, not mutually exclusive).
        #
        # Restricted to trading hours (START_TRADES..END_TRADES): the
        # first hourly post lands right after "start of trading" opens,
        # and no hourly post goes out once "end of trading" has closed
        # for the day -- it resumes automatically at the next start.

        hourly_report_window_start = (
            START_TRADES_HOUR * 60
            + START_TRADES_MINUTE
        )

        hourly_report_window_end = (
            END_TRADES_HOUR * 60
            + END_TRADES_MINUTE
        )

        hourly_report_current_minute = current_minutes()

        # The morning's very first board in the channel must always be the
        # regular price board (sourced from Taghizadegan / manual entry),
        # never this hourly text report. So the hourly report is only
        # allowed to fire once at least one such board has already been
        # published today -- state["updated_at"] is stamped exactly when
        # a full price-board transaction commits (see publish_price_
        # transaction above), so "today" on that timestamp is proof a
        # board already went out today.
        todays_board_already_published = False
        last_board_committed_at = state.get("updated_at")
        if last_board_committed_at:
            try:
                last_board_committed_date = datetime.fromisoformat(
                    last_board_committed_at
                ).date()
                if last_board_committed_date == iran_now().date():
                    todays_board_already_published = True
            except Exception:
                todays_board_already_published = False

        if (
            hourly_report_window_start
            <= hourly_report_current_minute
            < hourly_report_window_end
            and todays_board_already_published
            and should_send_hourly(state, "live_market")
        ):

            try:
                hourly_gold_snapshot = await get_board_gold_snapshot()
            except Exception as error:
                log.warning("HOURLY REPORT GOLD SNAPSHOT FAILED | %s", error)
                hourly_gold_snapshot = None

            try:
                hourly_silver_dollar = await get_board_silver_dollar_snapshot()
            except Exception as error:
                log.warning("HOURLY REPORT SILVER/DOLLAR FAILED | %s", error)
                hourly_silver_dollar = None

            hourly_gold_18 = (
                hourly_gold_snapshot.get("gold_18")
                if hourly_gold_snapshot else None
            )
            hourly_gold_ounce = (
                hourly_gold_snapshot.get("gold_ounce")
                if hourly_gold_snapshot else None
            )
            hourly_silver_ounce = (
                hourly_silver_dollar.get("ounce")
                if hourly_silver_dollar else None
            )
            hourly_dollar = (
                hourly_silver_dollar.get("tehran")
                if hourly_silver_dollar else None
            )

            if (
                hourly_gold_18 is not None
                or hourly_gold_ounce is not None
                or hourly_silver_ounce is not None
                or hourly_dollar is not None
            ):
                ...
                if hourly_gold_18 is not None:
                    hourly_lines.extend([
                        "🥇 طلای ۱۸ عیار",
                        f"💰 {format_price(hourly_gold_18)} تومان",
                        "",
                    ])

                if hourly_gold_ounce is not None:
                    hourly_lines.extend([
                        "🌍 انس طلا",
                        f"💵 {hourly_gold_ounce:.2f} دلار",
                        "",
                    ])

                if hourly_silver_ounce is not None:
                    hourly_lines.extend([
                        "🥈 انس نقره",
                        f"💵 {hourly_silver_ounce:.2f} دلار",
                        "",
                    ])

                if hourly_dollar is not None:
                    hourly_lines.extend([
                        "💵 دلار تهران",
                        f"💰 {format_price(hourly_dollar)} تومان",
                    ])
                ...
                try:
                    await send_text_post(
                        client,
                        target,
                        "\n".join(hourly_lines).strip()
                    )

                    mark_hourly_sent(state, "live_market")
                    save_state(state)

                    log.info(
                        "HOURLY LIVE MARKET REPORT SENT | GOLD18=%s | "
                        "GOLD_OUNCE=%s | SILVER_OUNCE=%s | DOLLAR=%s",
                        hourly_gold_18,
                        hourly_gold_ounce,
                        hourly_silver_ounce,
                        hourly_dollar,
                    ...
                    log.exception(
                        "HOURLY LIVE MARKET REPORT SEND FAILED: %s",
                        error
                    )
