...
END_STICKER_UPSCALE_SIZE = (1536, 1536)
...
def is_duplicate_news(
    ...
    if current_signature:
        ...
        for old_signature in history_fingerprints:
            ...
            if similarity >= NEWS_CONTENT_DUPLICATE_LIMIT:
                log.info(
                    "DUPLICATE NEWS SKIPPED | CONTENT=%.3f | %s",
                    similarity,
                    title
                )
                return True

            # Strong cross-channel repost guard.
            shared = len(current_words & old_words)

            if shared >= 10 and similarity >= 0.50:
                log.info(
                    "DUPLICATE NEWS SKIPPED | STORY=%.3f | SHARED=%s | %s",
                    similarity,
                    shared,
                    title
                )
...
async def send_sticker(
    ...
    """
    Send a WebP as a Telegram sticker.

    If upscale_size is supplied, the artwork is resized only at send-time;
    the source file itself is not modified.  This keeps the repository asset
    unchanged while allowing the Telegram sticker to render larger.
    """
    ...
    try:
        ...
        if upscale_size:
            with Image.open(sticker_path) as im:
                ...
                upload_source = upload_bytes

        if upload_bytes is not None:
            with Image.open(upload_bytes) as sticker_image:
        ...
        log.info(
            "TELEGRAM STICKER SENT | id=%s | file=%s | size=%sx%s",
            sent.id,
            sticker_path.name,
            sticker_width,
            sticker_height
