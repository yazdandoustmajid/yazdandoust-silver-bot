...
async def send_market_poll(
    ...
    poll = Poll(
        id=0,
        question="📊 نبض بازار نقره | پیش‌بینی فردا",
        answers=[
            PollAnswer(
                text="🟢 صعودی",
                option=b"\x01"
...
async def send_sticker(
    ...
    try:
        ...
        upload_source = str(sticker_path)
        upload_buffer = None
        ...
        try:
            with Image.open(sticker_path) as sticker_image:
                ...
                if upscale_size:
                    target_width, target_height = upscale_size

                    # Preserve the original artwork exactly; only increase
                    # its pixel dimensions using a high-quality resampler.
                    if (sticker_width, sticker_height) != (
                        target_width,
                        target_height
                    ):
                        resized = sticker_image.resize(
                        ...
                        upload_buffer.name = sticker_path.name
                        upload_source = upload_buffer

                        sticker_width = target_width
                        sticker_height = target_height
