...
async def process_price_menu_message(client, state, admin_id, raw):
    ...
    if stage == "duration":
        ...
        # Keep the existing 995 mithqal calculation in sync unless there
        # is already a dedicated manual mithqal override.
        if "mithqal_995" not in manual_price_overrides(state):
            products["mithqal_995"] = (
                round(products["shot_995"] * MITHQAL_GRAMS / 100) * 100
            )
