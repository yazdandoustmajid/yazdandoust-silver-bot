...
def _make_poll(
    ...
    for text, option in options:
        ...
        if rich:
            answer_text = _poll_rich_text(text)
    ...
    if rich:
        poll_question = _poll_rich_text(question)
...
def build_telegram_poll(
    ...
    """Poll سازگار با نسخه Telethon نصب‌شده می‌سازد."""
    ...
    for rich in (False, True):
        ...
        try:
            ...
            # تست سریالایز؛ اگر ساختار فیلدها با لایه فعلی نخواند همین‌جا
            # خطا می‌دهد و حالت بعدی امتحان می‌شود.
            bytes(poll)
