...
def sales_send(chat_id,text,keyboard=None,remove=False):
    if not BOT_TOKEN:
        log.warning("SALES SEND FAILED | BOT_TOKEN missing | chat=%s", chat_id)
        return False
    ...
    try:
        ...
        if not r.ok or not data.get("ok",False):
            log.warning(
                ...
                chat_id, r.status_code, data.get("ok"),
...
def sales_customer_handle(state,message):
    ...
    log.info(
        ...
        uid, cid, text[:120], c.get("stage"), active.get("status") if active else None,
    )
    if photos:
