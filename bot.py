...
async def _main_locked():
    ...
    try:
        ...
        # =================================================
        # Poll window: 22:10-23:30 (Tehran time).
        # GitHub Actions "schedule" runs are best-effort and can be
        # delayed by anywhere from a few minutes to well over an hour
        # during high load, especially for a */5 cron — and a missed
        # scheduled tick is simply dropped, never re-run automatically.
        # A narrow ~20 minute window was getting skipped entirely on
        # delayed days, which is why the poll was never appearing. This
        # wider 80-minute window gives far more 5-minute ticks a chance
        # to catch it, while should_send_daily() below still guarantees
