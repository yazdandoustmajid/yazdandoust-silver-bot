...
async def process_manual_price_commands(client, state):
    ...
    try:
        last_id = int(state.get(MANUAL_COMMAND_LAST_ID_KEY, 0) or 0)
    except Exception:
        last_id = 0
    ...
    try:
        ...
        log.warning("MANUAL PRICE COMMAND READ FAILED: %s", error)
        return

    if not messages:
        return

    # On the first run after enabling this feature, establish a checkpoint
    # without executing old messages.
    if last_id == 0:
        newest = max(int(m.id) for m in messages if getattr(m, "id", None))
        state[MANUAL_COMMAND_LAST_ID_KEY] = newest
        save_state(state)
        return
    ...
    for message in messages:
        ...
        if getattr(message, "sender_id", None) not in (admin_id,):
            continue

        raw = (getattr(message, "message", None) or "").strip()
        if not raw:
            continue
        ...
        try:
            if command in ("/price", "/manual_price"):
                if len(parts) not in (3, 4):
                    await client.send_message(admin_id, manual_price_help_text())
                    continue

                product_key = MANUAL_PRICE_ALIASES.get(parts[1].lower())
                if not product_key:
                ...
                if minutes is not None:
                    expires_at = (iran_now() + timedelta(minutes=minutes)).isoformat()
            ...
            elif command in ("/pricehelp", "/manualhelp"):
                await client.send_message(admin_id, manual_price_help_text())
    ...
    state[MANUAL_COMMAND_LAST_ID_KEY] = newest_id
    state[MANUAL_PRICE_OVERRIDES_KEY] = overrides

    if changed or newest_id != last_id:
        save_state(state)
