...
def _prepare_telegram_bot_polling():
    """Prepare Bot API long-polling safely before reading getUpdates.

    If a webhook was previously configured for this bot, Telegram refuses
    getUpdates with a conflict. This bot runs on GitHub Actions and therefore
    intentionally uses polling, so an old webhook must be removed without
    ...
    try:
        ...
        if data.get("ok"):
            ...
            if webhook:
                log.warning(
                    "TELEGRAM WEBHOOK DETECTED | removing it so getUpdates can work"
                )
...
async def process_price_menu_message(client, state, admin_id, raw):
    ...
    if raw == "/start":
        ...
        log.info("PRICE MENU START SENT")
        return True
...
def sales_send(chat_id,text,keyboard=None,remove=False):
    ...
    if keyboard is not None: payload["reply_markup"]=json.dumps({"keyboard":keyboard,"resize_keyboard":True,"one_time_keyboard":False},ensure_ascii=False)
    elif remove: payload["reply_markup"]=json.dumps({"remove_keyboard":True})
    try:
        r=requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",data=payload,timeout=15); return bool(r.ok)
    except Exception as e: log.warning("SALES SEND FAILED | %s",e); return False

def sales_send_photo(chat_id,photo_id,caption):
    try:
        r=requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendPhoto",data={"chat_id":int(chat_id),"photo":photo_id,"caption":caption},timeout=20); return bool(r.ok)
    except Exception as e: log.warning("SALES SEND PHOTO FAILED | %s",e); return False
...
def sales_download_file(file_id):
    try:
        r=requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/getFile",params={"file_id":file_id},timeout=15); r.raise_for_status(); path=r.json()["result"]["file_path"]
        b=requests.get(f"https://api.telegram.org/file/bot{BOT_TOKEN}/{path}",timeout=20); b.raise_for_status()
        fp=BASE/".sales_receipt_ocr.jpg"; fp.write_bytes(b.content); return fp
    except Exception as e: log.warning("SALES RECEIPT DOWNLOAD FAILED | %s",e); return None
...
def sales_admin_help():
    return ("🛠 پنل مدیریت فروش\n\n/sales_orders — سفارش‌های اخیر\n/sales_stats — گزارش فروش\n"
            ...
            "/sales_cancel ORDER — لغو\n/sales_help — همین راهنما")

def sales_admin_handle(state,raw):
    parts=raw.split(); cmd=parts[0].lower().split("@",1)[0] if parts else ""
    if cmd=="/sales_help" or raw=="🛠 مدیریت فروش": return sales_admin_help()
    if cmd=="/sales_price": return f"💰 قیمت ساچمه ۹۹۵: {sales_money(sales_unit_price(state) or 0)}"
    if cmd=="/sales_rates": return "📊 درصدها\n۱۰ تا کمتر از ۵۰: ۷٪\n۵۰ تا کمتر از ۱۰۰: ۶٪\n۱۰۰ تا کمتر از ۱۵۰: ۵٪\n۱۵۰ تا کمتر از ۲۰۰: ۴٪\n۲۰۰ تا کمتر از ۳۰۰: ۲٪\n۳۰۰ تا کمتر از ۵۰۰: ۱٪\n۵۰۰ تا ۱۰۰۰: ۰٪"
    if cmd=="/sales_inventory" and len(parts)>1:
        try: state[SALES_INVENTORY_KEY]=int(re.sub(r"\D","",normalize_digits(parts[1]))); save_state(state); return f"✅ موجودی روی {state[SALES_INVENTORY_KEY]:,} گرم تنظیم شد."
        except: return "❌ مقدار موجودی نامعتبر است."
    if cmd=="/sales_coupon" and len(parts)>=3:
        code=parts[1].upper(); pct=int(re.sub(r"\D","",normalize_digits(parts[2]))); sales_coupons(state)[code]={"percent":min(100,pct),"created_at":iran_now().isoformat()}; save_state(state); return f"✅ کد {code} با تخفیف {pct}٪ ساخته شد."
    if cmd=="/sales_coupon_off" and len(parts)>1:
        sales_coupons(state).pop(parts[1].upper(),None); save_state(state); return "✅ کد حذف شد."
    if cmd=="/sales_orders":
        os=sales_orders(state)[-10:]; return "\n\n".join(sales_order_summary(o) for o in reversed(os)) if os else "📦 سفارشی ثبت نشده."
    if cmd=="/sales_stats":
        day=today_key(); os=[o for o in sales_orders(state) if o.get("approved_day")==day and o.get("status") in {"approved","preparing","shipped","completed"}]; grams=sum(int(o.get("weight",0)) for o in os); amount=sum(int(o.get("total",0)) for o in os); return f"📊 گزارش امروز\n\nسفارش تأییدشده: {len(os)}\nوزن: {grams:,} گرم\nفروش: {sales_money(amount)}\nمیانگین سفارش: {grams//len(os) if os else 0:,} گرم"
    if cmd in {"/sales_approve","/sales_reject","/sales_prepare","/sales_ship","/sales_complete","/sales_cancel"} and len(parts)>1:
        oid=parts[1].upper(); o=next((x for x in sales_orders(state) if x.get("order_id")==oid),None)
        if not o:return "❌ سفارش پیدا نشد."
        if cmd=="/sales_approve":
            if o.get("status") not in {"receipt_submitted","awaiting_receipt"}: return "❌ این سفارش آماده تأیید پرداخت نیست."
            inv=sales_inventory(state)
            if inv is not None and int(o["weight"])>inv:return "❌ موجودی کافی نیست."
            o["status"]="approved"; o["approved_day"]=today_key();
            if inv is not None: state[SALES_INVENTORY_KEY]=inv-int(o["weight"])
            save_state(state); sales_send(o["telegram_id"],"✅ پرداخت شما تأیید شد.\n\n"+sales_order_summary(o),[["📦 سفارش‌های من","👤 حساب من"]]); return "✅ پرداخت تأیید شد."
        if cmd=="/sales_reject": o["status"]="rejected"; save_state(state); sales_send(o["telegram_id"],"❌ رسید/پرداخت سفارش شما تأیید نشد. لطفاً با پشتیبانی تماس بگیرید.",sales_keyboard()); return "✅ سفارش رد شد."
        if cmd=="/sales_prepare": o["status"]="preparing"; save_state(state); sales_send(o["telegram_id"],"📦 سفارش شما در حال آماده‌سازی است.",sales_keyboard()); return "✅ آماده‌سازی ثبت شد."
        if cmd=="/sales_ship":
            if len(parts)<3:return "❌ کد رهگیری را هم وارد کنید."
            o["status"]="shipped"; o["tracking_code"]=" ".join(parts[2:]); o["carrier"]=SHIPPING_CARRIER; save_state(state); sales_send(o["telegram_id"],f"🚚 سفارش شما ارسال شد.\nشرکت: {SHIPPING_CARRIER}\nکد رهگیری: {o['tracking_code']}",sales_keyboard()); return "✅ ارسال ثبت شد."
        if cmd=="/sales_complete": o["status"]="completed"; save_state(state); sales_send(o["telegram_id"],"✅ سفارش شما تکمیل شد. ممنون از اعتماد شما.",sales_keyboard()); return "✅ تکمیل شد."
        if cmd=="/sales_cancel": o["status"]="cancelled"; save_state(state); sales_send(o["telegram_id"],"❌ سفارش لغو شد.",sales_keyboard()); return "✅ لغو شد."
    return None
...
def sales_customer_handle(state,message):
    ...
    c=sales_customer(state,uid); text=(message.get("text") or "").strip(); photos=message.get("photo") or []
    active=sales_active(state,uid)
    if photos:
        if not active or active.get("status")!="awaiting_receipt": sales_send(cid,"⚠️ سفارش فعالی برای دریافت رسید ندارید.",sales_keyboard()); return True
        active["receipt_file_id"]=photos[-1]["file_id"]; active["status"]="receipt_submitted"; active["receipt_submitted_at"]=iran_now().isoformat(); save_state(state)
        ocr=sales_ocr_amount(active["receipt_file_id"]); extra=f"🤖 مبلغ OCR: {sales_money(ocr)}" if ocr else "🤖 OCR مبلغ را تشخیص نداد."
        sales_send(cid,"✅ رسید دریافت شد. پس از بررسی مدیریت نتیجه اعلام می‌شود.",sales_keyboard()); sales_notify_admin(state,active,active["receipt_file_id"],extra); return True
    ...
    if active:
        if active["status"]=="awaiting_name":
            ...
            c["name"]=text; active["status"]="awaiting_phone"; save_state(state); sales_send(cid,"📱 شماره موبایل را وارد کنید:",[ ["❌ لغو"] ]); return True
        if active["status"]=="awaiting_phone":
            ...
            if adjusted + current_weight > SALES_DAILY_LIMIT_GRAMS:
                ...
                return True
            c["phone"]=phone; active["customer_key"]=phone; active["status"]="awaiting_address"; save_state(state); sales_send(cid,"📍 آدرس کامل تحویل را وارد کنید:",[ ["❌ لغو"] ]); return True
        if active["status"]=="awaiting_address":
            ...
            if PAYMENT_URL:sales_send(cid,"🌐 پرداخت آنلاین:",inline=None); sales_send(cid,PAYMENT_URL)
            return True
...
async def process_manual_price_commands(client, state):
    ...
    if not BOT_TOKEN:
        log.warning(
            "MANUAL PRICE / SALES DISABLED | BOT_TOKEN is missing"
        )
        return

    # getUpdates and webhook delivery are mutually exclusive. Make the
    # polling mode explicit on every runner process, while preserving queued
    # updates.
    ...
    if ADMIN_TELEGRAM_ID:
        ...
        log.warning(
            "ADMIN PRICE MENU DISABLED | ADMIN_TELEGRAM_ID is missing | "
            "customer /start and sales updates remain enabled"
        )
    ...
    for update in updates:
        ...
        # A private chat with the configured admin ID is treated as admin; all
        # other chats stay in the customer/sales flow.
        is_admin = (
            admin_id is not None
            and (sender_id == admin_id or chat_id == admin_id)
        )
        ...
        log.info(
            "ADMIN UPDATE ACCEPTED | chat_id=%s | sender_id=%s | text=%s",
            chat_id,
            sender_id,
            raw[:80],
        )

        # Use the actual private chat id when available. This avoids a silent
        # mismatch if the configured admin ID is correct but Telegram reports
        # the conversation under the chat object.
        admin_chat_id = chat_id or admin_id
...
LIVE_PRICE_MONITOR_SECONDS = 290
