...
# This file is the PRICE BOT only.
# @Yazdandoustpricebot must use PRICE_BOT_TOKEN.
BOT_ROLE = "price"
BOT_TOKEN = os.getenv("PRICE_BOT_TOKEN", "").strip()

PRICE_BOT_USERNAME = (
    os.getenv("PRICE_BOT_USERNAME", "Yazdandoustpricebot")
    .strip().lstrip("@").lower()
)
...
def create_board(
    ...
    for box, value in zip(
        ...
        # قیمت‌ها در نسخه قبلی بیش از حد کوچک بودند.
        # اندازه فونت را تا حد ممکن بزرگ می‌کنیم و fit_font_to_box
        # فقط در صورت نیاز به خاطر عرض/ارتفاع باکس آن را کوچک می‌کند.
        # اعداد تابلو باید در نمایش موبایل هم کاملاً خوانا باشند.
        # باکس عدد حدود 395x80 پیکسل است؛ بنابراین ابتدا فونت را خیلی
        # بزرگ انتخاب می‌کنیم و fit_font_to_box فقط در صورت نیاز آن را
        # برای جا شدن کامل داخل باکس کاهش می‌دهد.
        if len(value) >= 10:
...
async def process_manual_price_commands(client, state):
    ...
    for update in updates:
        ...
        # PRICE BOT IS ADMIN-ONLY.
        # Customer orders, receipts, coupons, and sales administration belong
        # exclusively to @Yazdandoustsilverbot.
        if not is_admin:
            log.info(
                "PRICE BOT IGNORED NON-ADMIN UPDATE | update_id=%s",
                update_id,
            )
            continue
        # Non-admin updates are intentionally ignored by the price bot.
        # The sales bot owns all customer conversations.
...
async def _main_locked():
    ...
    try:
        ...
        # Hard safety check: this file must never run as the sales bot.
        bot_me = await client.get_me()
        actual_bot_username = (
            str(getattr(bot_me, "username", "") or "")
            .strip().lstrip("@").lower()
        )
        if (
            PRICE_BOT_USERNAME
            and actual_bot_username != PRICE_BOT_USERNAME
        ):
            raise RuntimeError(
                "PRICE_BOT_TOKEN به ربات اشتباه متصل است. "
                f"ربات فعلی: @{actual_bot_username or 'نامشخص'} | "
                f"ربات مورد انتظار: @{PRICE_BOT_USERNAME}. "
                "Secret با نام PRICE_BOT_TOKEN را بررسی کنید."
            )

        log.info(
            "PRICE BOT IDENTITY VERIFIED | username=@%s",
            actual_bot_username,
        )
        ...
        if (
            ...
            # GitHub Actions ممکن است اجرای زمان‌بندی‌شده را با تأخیر شروع کند.
            # پیام صبح بخیر را تا قبل از شروع بازه قیمت‌گذاری نگه می‌داریم تا
            # اگر اجرای 08:00 از دست رفت، در اولین اجرای 08:xx/09:xx/10:xx
            # همچنان ارسال شود. should_send_daily از ارسال تکراری جلوگیری می‌کند.
            current_minute
            <
            PRICE_START_HOUR * 60
            + PRICE_START_MINUTE
