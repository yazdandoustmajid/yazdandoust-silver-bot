...
START_STICKER = BASE / "Start_Traders(2).webp"
END_STICKER = BASE / "End_Traders(2).webp"
...
def build_market_analysis_snapshot(
    ...
    for key in (
        ...
        # Yahoo داده‌های روزانه می‌دهد؛ برای گزارش دو‌روزه،
        # تغییر نسبت به دو کندل معاملاتی قبل را نیز نگه می‌داریم.
        if len(rows) >= 3:
...
def market_analysis_text_fallback(
    ...
    """
    نسخه بدون AI برای گزارش دو‌روزه بازار نقره.
    فقط از داده‌های واقعی snapshot استفاده می‌کند.
    """

    def value(key, decimals=2):
    ...
    def levels(key, decimals=2):
        item = snapshot.get(key, {})
        lv = item.get("levels", {})
        return (
            f"حمایت: {format_level(lv.get('support'), decimals)} | "
            f"مقاومت: {format_level(lv.get('resistance'), decimals)}"
        )

    silver = snapshot.get("silver", {})
    gold = snapshot.get("gold", {})
    dollar = snapshot.get("tehran_dollar", {})
    dxy = snapshot.get("dxy", {})
    oil = snapshot.get("oil", {})
    us10y = snapshot.get("us10y", {})
    ratio = snapshot.get("gold_silver_ratio")

    dxy_change = dxy.get("two_day_change_percent")
    us10y_change = us10y.get("two_day_change_percent")
    gold_change = gold.get("two_day_change_percent")

    ratio_view = (
        "کاهش نسبت طلا به نقره به نفع عملکرد نسبی نقره است."
        if ratio is not None
        else "داده کافی برای نسبت طلا به نقره نداریم."
    )

    if dxy_change is not None and dxy_change < 0:
        dxy_view = "ضعف دلار در این بازه به نفع نقره بوده است."
    elif dxy_change is not None and dxy_change > 0:
        dxy_view = "تقویت دلار در این بازه یک عامل فشار بر نقره است."
    else:
        dxy_view = "داده کافی برای تعیین اثر دو‌روزه دلار نداریم."

    if us10y_change is not None and us10y_change > 0:
        rates_view = "افزایش بازدهی اوراق ۱۰ساله برای نقره عامل محدودکننده است."
    elif us10y_change is not None and us10y_change < 0:
        rates_view = "افت بازدهی اوراق ۱۰ساله فضای بهتری برای نقره ایجاد می‌کند."
    else:
        rates_view = "بازدهی اوراق ۱۰ساله تغییر تعیین‌کننده‌ای نشان نمی‌دهد."

    if (
        silver.get("current") is not None
        and gold_change is not None
        and dxy_change is not None
        and us10y_change is not None
    ):
        positive_score = (
            (gold_change > 0)
            + (dxy_change < 0)
    ...
    return (
        "📊 گزارش دو‌روزه بازار نقره | یزدان‌دوست\n"
        "━━━━━━━━━━━━━━\n\n"

        f"🥈 XAGUSD | انس نقره: {value('silver')} دلار\n"
        f"تغییر دو‌روزه: {change('silver')}\n"
        f"{levels('silver')}\n\n"

        f"🥇 XAUUSD | انس طلا: {value('gold')} دلار\n"
        f"تغییر دو‌روزه: {change('gold')}\n"
        f"{levels('gold')}\n\n"

        f"💵 دلار تهران: {value('tehran_dollar', 0)} تومان\n"
        f"تغییر دو‌روزه: {change('tehran_dollar')}\n"
        f"{levels('tehran_dollar', 0)}\n\n"

        f"🌐 DXY: {value('dxy')} | تغییر دو‌روزه: {change('dxy')}\n"
        f"🛢 نفت WTI: {value('oil')} دلار | تغییر دو‌روزه: {change('oil')}\n"
        f"🇺🇸 US10Y: {value('us10y')}٪ | تغییر دو‌روزه: {change('us10y')}\n"
        f"📈 S&P 500: {value('sp500')} | تغییر دو‌روزه: {change('sp500')}\n"
        f"🌪 VIX: {value('vix')} | تغییر دو‌روزه: {change('vix')}\n"
        f"⚖️ نسبت طلا/نقره: {format_level(ratio)}\n\n"

        f"🟢 وضعیت کوتاه‌مدت نقره: {view}\n"
        f"{dxy_view}\n"
        f"{rates_view}\n"
        f"{ratio_view}\n\n"

        "🔎 جمع‌بندی: برای ادامه حرکت صعودی نقره، "
        "هم‌زمانی ضعف دلار، ثبات یا کاهش بازدهی اوراق و قدرت طلا "
        "ترکیب مساعدتری است؛ در مقابل، تقویت دلار و افزایش بازدهی "
        "می‌تواند فشار اصلاحی ایجاد کند.\n\n"
...
def ai_market_analysis_sync(
    ...
    prompt = f"""
برای کانال «یزدان‌دوست» یک «گزارش دو‌روزه بازار نقره» بنویس.
...
1) XAGUSD / انس نقره: قیمت فعلی، تغییر دو‌روزه و حمایت/مقاومت.
2) XAUUSD / انس طلا: قیمت فعلی، تغییر دو‌روزه و حمایت/مقاومت.
3) دلار تهران: قیمت فعلی، تغییر دو‌روزه و حمایت/مقاومت؛ اگر داده کافی نیست صریحاً بگو.
4) DXY: قیمت فعلی، تغییر دو‌روزه و اثر آن بر نقره.
5) نفت WTI: قیمت فعلی، تغییر دو‌روزه و اثر احتمالی آن بر نقره.
6) US10Y: بازده فعلی، تغییر دو‌روزه و اثر آن بر نقره.
7) نسبت طلا به نقره: مقدار فعلی و اثر افزایش/کاهش آن بر عملکرد نسبی نقره.
8) S&P 500 و VIX: وضعیت فعلی، تغییر دو‌روزه و اثر احتمالی آنها بر نقره.
9) مهم‌ترین عوامل صعودی و نزولی نقره را جداگانه و کوتاه بیان کن.
10) وضعیت کلی نقره را «مثبت»، «منفی» یا «خنثی» اعلام کن و دلیل آن را بر اساس داده‌ها توضیح بده.

قواعد:
- فقط از داده‌های ورودی استفاده کن؛ عدد، حمایت، مقاومت یا تغییر قیمتی اختراع نکن.
- اگر داده‌ای موجود نیست، بنویس «داده کافی نداریم».
- منظور از تغییر دو‌روزه، تغییر نسبت به حدود دو کندل روزانه معاملاتی قبل است.
    ...
    try:
        ...
        if media_path:
            ...
            if len(caption) <= NEWS_MEDIA_CAPTION_LIMIT:
                ...
                # Telegram media captions are capped at 1024
                # characters (vs 4096 for plain text messages), so a
                # caption longer than that can't fit on the video
                # itself. Post the media with a short caption, then
                # the full write-up right under it as its own message.
                short_caption = build_short_news_caption(
                    article
                )
...
async def send_sticker(
    ...
    try:
        # Telegram identifies a sticker by the DocumentAttributeSticker
        # attached to the document, together with DocumentAttributeImageSize
        # so clients know its rendered size. force_document=True must NOT be
        # used here: on the raw Telegram API this maps to the force_file
        # flag, which explicitly tells clients "always render this as a
        # generic downloadable file", overriding the sticker attributes and
        # is exactly why the sticker was showing up as a plain file.
        try:
            with Image.open(sticker_path) as sticker_image:
                sticker_width, sticker_height = sticker_image.size

        except Exception:
            sticker_width, sticker_height = 512, 512
...
async def _main_locked():
    ...
    try:
        ...
        # =================================================
        # Window widened from 15 min (09:45-10:00) to 09:45-11:30.
