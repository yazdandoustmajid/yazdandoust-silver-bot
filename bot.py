...
async def send_daily_silver_analysis(
    ...
    # مقدارهای تحلیل نموداری را با آخرین قیمت واقعی ساچمه ۹۹۵ هماهنگ می‌کنیم.
    # build_market_analysis_snapshot فقط داده‌های TradingView را می‌سازد،
    # بنابراین shot_995 باید قبل از استفاده به‌صورت جداگانه به snapshot اضافه شود.
    shot_raw = products.get("shot_995") if isinstance(products, dict) else None

    if shot_raw is None:
        log.error(
            "DAILY SILVER ANALYSIS SKIPPED | shot_995 missing from products"
        )
        return None
