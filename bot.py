...
async def run_sales_only_bot(state, duration_seconds=270):
    ...
    if not _prepare_telegram_bot_polling():
        raise RuntimeError(
            "SALES BOT POLLING PREPARATION FAILED. "
            "Check BOT_TOKEN and Telegram webhook/getUpdates status."
        )
    ...
    log.info(
        "SALES BOT ONLY STARTED | duration=%ss | BOT_ROLE=%s",
        duration_seconds,
        BOT_ROLE,
    )
...
async def _main_locked():
    ...
    # Price bot uses Telethon and therefore needs API_ID/API_HASH.
    # Sales bot uses only Telegram Bot API getUpdates and must not require
