...
def sales_customer_handle(state,message):
    ...
    if photos or receipt_document:
        ...
        return True
    if text == "🛒 خرید ساچمه" or (text and text.split()[0].split("@")[0].lower() == "/start"):
        c["stage"]="weight"
        save_state(state)
        sent = sales_send(cid,"⚖️ وزن مورد نظر را انتخاب کنید:",sales_weight_keyboard())
        log.info("SALES START RECEIVED | uid=%s | chat=%s | send_weight_menu=%s", uid, cid, sent)
        return True
...
async def process_manual_price_commands(client, state):
    ...
    log.info(
        "BOT API UPDATE CONSUMER READY | ADMIN_AND_CUSTOMERS=ENABLED"
    )
    ...
    for update in updates:
        ...
        #
        # Customer sales messages use the same getUpdates consumer, avoiding
        # Telegram getUpdates conflicts.
        if not is_admin:
            try:
                log.info(
                    "CUSTOMER UPDATE RECEIVED | update_id=%s | sender=%s | chat=%s | text=%s",
                    update_id,
                    sender_id,
                    chat_id,
                    (message.get("text") or message.get("caption") or "")[:80],
                )
                if sales_customer_handle(state, message):
                    ...
                    changed = True
                    log.info(
                        "CUSTOMER UPDATE HANDLED | update_id=%s | chat=%s",
                        update_id,
                        chat_id,
                    )
                    continue
                log.info(
                    "CUSTOMER UPDATE NOT HANDLED | update_id=%s | chat=%s",
                    update_id,
                    chat_id,
                )
            except Exception as error:
