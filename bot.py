...
def sales_daily_approved(state,identity):
    ...
    for o in sales_orders(state):
        ...
        if o.get("status") in {"approved","preparing","shipped","completed"}: total+=int(o.get("weight",0))
    return total
...
def sales_order_new(state,uid,w):
    c=sales_customer(state,uid); identity=c.get("phone") or str(uid)
    used=sales_daily_approved(state,identity); remaining=max(0,SALES_DAILY_LIMIT_GRAMS-used)
    if w>remaining: return None,f"⚠️ سقف خرید روزانه شما {SALES_DAILY_LIMIT_GRAMS:,} گرم است. امروز {used:,} گرم تأیید شده و {remaining:,} گرم باقی مانده."
