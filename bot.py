...
def apply_manual_price_overrides(state, products):
    ...
    # If the 995 per-gram price is manually controlled, keep the mithqal
    # consistent with the same 995 price unless the user explicitly set
    # mithqal itself.
    if "shot_995" in overrides and "mithqal_995" not in overrides:
        result["mithqal_995"] = (
            round(result["shot_995"] * MITHQAL_GRAMS / 100) * 100
        )

    return result

def apply_manual_rate_overrides(state, rate):
    """Apply persistent manual overrides for dollar/ounce values."""
    overrides = cleanup_manual_price_overrides(state)
    if not overrides:
        return rate
    if rate is None:
...
# MANUAL PRICE MENU
# =========================================================
MANUAL_MENU_DRAFT_KEY = "manual_price_menu_draft"

def _manual_menu_keyboard():
    return {
        "inline_keyboard": [
            [
                {"text": "🥈 ساچمه ۹۹۵", "callback_data": "mp:set:shot_995"},
                {"text": "🧱 شمش", "callback_data": "mp:set:nader_9999"},
            ],
            [
                {"text": "💵 دلار", "callback_data": "mp:set:tehran"},
                {"text": "🌍 انس", "callback_data": "mp:set:ounce"},
            ],
            [
                {"text": "✅ تأیید و ارسال تابلو", "callback_data": "mp:confirm"},
            ],
            [
                {"text": "❌ لغو تغییرات", "callback_data": "mp:cancel"},
            ],
        ]
    }

def _manual_menu_text(state):
    draft = state.get(MANUAL_MENU_DRAFT_KEY)
    if not isinstance(draft, dict):
        draft = {}
    current_rate = get_saved_rate(state) or {}
    current_products = get_saved_products(state) or {}
    values = {
        "shot_995": draft.get("shot_995", current_products.get("shot_995")),
        "nader_9999": draft.get("nader_9999", current_products.get("nader_9999")),
        "tehran": draft.get("tehran", current_rate.get("tehran")),
        "ounce": draft.get("ounce", current_rate.get("ounce")),
    }
    def fmt(key):
    ...
    return (
        "🛠 <b>مدیریت قیمت دستی</b>\n"
        "━━━━━━━━━━━━━━\n\n"
        f"🥈 ساچمه ۹۹۵: <b>{fmt('shot_995')}</b> تومان\n"
        f"🧱 شمش: <b>{fmt('nader_9999')}</b> تومان\n"
        f"💵 دلار: <b>{fmt('tehran')}</b> تومان\n"
        f"🌍 انس: <b>{fmt('ounce')}</b>\n\n"
        "هر تعداد موردی را که خواستی تغییر بده.\n"
        "در پایان فقط «تأیید و ارسال تابلو» را بزن.\n\n"
        "⚠️ تا قبل از تأیید، هیچ تغییری در کانال ارسال نمی‌شود."
    )

def _manual_menu_draft_from_state(state):
    draft = state.get(MANUAL_MENU_DRAFT_KEY)
    if isinstance(draft, dict):
        return dict(draft)
    return {}

def _manual_menu_save(state, draft):
    state[MANUAL_MENU_DRAFT_KEY] = dict(draft)
    save_state(state)

def _manual_menu_clear(state):
    state.pop(MANUAL_MENU_DRAFT_KEY, None)
    save_state(state)

def _manual_menu_bot_api(token, method, payload):
...
def _manual_menu_send(token, chat_id, state, extra_text=None):
    text = _manual_menu_text(state)
    if extra_text:
        text = extra_text + "\n\n" + text
    return _manual_menu_bot_api(token, "sendMessage", {
        "chat_id": chat_id,
        "text": text,
        "parse_mode": "HTML",
        "reply_markup": _manual_menu_keyboard(),
    })

def _manual_menu_answer_callback(token, callback_id):
    try:
        _manual_menu_bot_api(token, "answerCallbackQuery", {"callback_query_id": callback_id})
    except Exception as error:
        log.warning("MANUAL MENU CALLBACK ANSWER FAILED | %s", error)
...
async def process_manual_price_commands(client, state):
    ...
    if not state.get(checkpoint_key, False):
        if updates:
            # Ignore messages that were already waiting when this feature
            # was first enabled.
            ...
            state[offset_key] = newest_update_id + 1

        state[checkpoint_key] = True
        save_state(state)

        log.info(
            "MANUAL PRICE UPDATE CHECKPOINT INITIALIZED | updates=%s",
            len(updates),
        )
        return
    ...
    for update in updates:
        ...
        from_user = (callback.get("from") or message.get("from") or {})
        ...
        # Only accept commands from the configured admin in the admin's
        # private chat. Other users' messages are consumed and ignored.
        if sender_id != admin_id or chat_id != admin_id:
            if callback.get("id"):
                _manual_menu_answer_callback(BOT_TOKEN, callback.get("id"))
            continue
        ...
        if callback:
            ...
            _manual_menu_answer_callback(BOT_TOKEN, callback_id)

            if data == "mp:open":
                _manual_menu_send(BOT_TOKEN, admin_id, state)
                continue

            if data.startswith("mp:set:"):
                key = data.split(":", 2)[2]
                if key not in ("shot_995", "nader_9999", "tehran", "ounce"):
                    continue
                draft = _manual_menu_draft_from_state(state)
                draft["awaiting"] = key
                _manual_menu_save(state, draft)
                labels = {
                    "shot_995": "🥈 ساچمه ۹۹۵",
                    "nader_9999": "🧱 شمش",
                    "tehran": "💵 دلار",
                ...
                await client.send_message(
                    admin_id,
                    f"✏️ قیمت جدید {labels[key]} را فقط به صورت عددی بفرست.\n"
                    f"مثال: <code>{'425000' if key != 'ounce' else '3200.50'}</code>\n\n"
                    "بعد از وارد کردن عدد، دوباره منو را می‌بینی و می‌توانی موارد دیگر را هم تغییر بدهی.\n"
                    "تا وقتی «تأیید و ارسال تابلو» را نزنی، هیچ پستی در کانال تغییر نمی‌کند.",
                    parse_mode="html"
                )
                continue
            ...
            if data == "mp:confirm":
                ...
                changed_keys = [k for k in draft if k in ("shot_995", "nader_9999", "tehran", "ounce")]
                if not changed_keys:
                    await client.send_message(admin_id, "ℹ️ هنوز هیچ قیمتی برای تغییر انتخاب نکرده‌ای.")
                    continue
                ...
                if rate is None:
                    try:
                        rate, _ = await asyncio.to_thread(find_latest_public_rate)
                    except Exception:
                        rate = None
                if products is None:
                    try:
                ...
                if rate is None or products is None:
                    await client.send_message(admin_id, "❌ اطلاعات پایه قیمت هنوز در ربات موجود نیست. ابتدا یک اجرای موفق قیمت انجام بده.")
                    continue
                ...
                for key in changed_keys:
                    ...
                    if key == "shot_995":
                        products["shot_995"] = int(value)
                        products["shot_package"] = int(value) * 1000
                    elif key == "nader_9999":
                        products["nader_9999"] = int(value)
                        products["nader_package"] = int(value) * 1000
                    elif key == "tehran":
                        rate["tehran"] = int(value)
                    elif key == "ounce":
                        rate["ounce"] = float(value)

                # Keep the board's 995 mithqal value consistent with a manual 995 change.
                if "shot_995" in changed_keys:
                    products["mithqal_995"] = round(products["shot_995"] * MITHQAL_GRAMS / 100) * 100

                state[MANUAL_PRICE_OVERRIDES_KEY] = overrides
                _manual_menu_clear(state)
                save_state(state)
                ...
                try:
                    ...
                    await client.send_message(
                        admin_id,
                        "✅ <b>تابلو با موفقیت بروزرسانی و در کانال ارسال شد.</b>\n\n"
                        "قیمت‌های انتخاب‌شده تا وقتی به حالت خودکار برنگردند، دستی باقی می‌مانند.\n"
                        "برای برگشت همه موارد: <code>/auto all</code>",
                        parse_mode="html"
                    )
                except Exception as error:
                    log.exception("MANUAL MENU PUBLISH FAILED | %s", error)
                    await client.send_message(admin_id, "❌ ارسال تابلو انجام نشد. خطا در لاگ GitHub ثبت شد.")
                continue

            if data == "mp:cancel":
                _manual_menu_clear(state)
                await client.send_message(admin_id, "❌ تغییرات لغو شد؛ هیچ قیمتی در کانال تغییر نکرد.")
                continue
        ...
        # -------------------------------------------------------------
        # /start opens the new multi-price menu.
        # -------------------------------------------------------------
        if command == "/start":
            _manual_menu_send(BOT_TOKEN, admin_id, state)
            continue

        # If the admin selected an item from the menu, a plain numeric
        # message is stored in the draft and NOTHING is published yet.
        draft = _manual_menu_draft_from_state(state)
        awaiting = draft.get("awaiting")
        if awaiting and not command.startswith("/"):
            try:
                cleaned = re.sub(r"[^0-9۰-۹٠-٩.]", "", raw)
                ...
                if awaiting == "ounce":
                    ...
                    value = int(float(cleaned.replace(".", "")))
                    if value <= 0:
                        raise ValueError
                ...
                _manual_menu_send(
                    ...
                    admin_id,
                    state,
                    f"✅ {labels[awaiting]} در پیش‌نویس روی <b>{value if awaiting == 'ounce' else format_price(value)}</b> تنظیم شد.\n"
                    "هنوز چیزی در کانال ارسال نشده است."
                )
                continue
            except Exception:
                await client.send_message(admin_id, "❌ عدد واردشده معتبر نیست. فقط عدد قیمت را بفرست.")
                continue
...
LIVE_PRICE_MONITOR_SECONDS = 180  # 3 minutes; leaves headroom inside a 5-minute
# cron so the whole job (news/report tasks + this loop) finishes comfortably
# under 5 minutes and the next scheduled run never has to queue behind it.
...
async def monitor_live_price_changes(
    ...
    while (
        ...
        try:
            ...
            raw_source_rate = dict(rate) if isinstance(rate, dict) else rate
