...
async def process_manual_price_commands(client, state):
    ...
    for update in updates:
        ...
        # Admin messages are handled by the real-time Telethon event handler.
        # Consume the Bot API copy only to advance the shared update offset.
        if is_admin:
            continue
...
# =========================================================
# The admin panel is received directly through the already-connected
# Telethon bot session.  This removes the most fragile part of the previous
# design: waiting for the shared Bot API getUpdates queue.  Customer sales
# messages continue through getUpdates below.
...
async def _main_locked():
    ...
    try:
        ...
        install_admin_event_handler(client, state, target)
        ...
        if ADMIN_TELEGRAM_ID:
            log.info(
                "ADMIN MENU CONFIGURED | ADMIN_TELEGRAM_ID=%s | REAL_TIME_EVENTS=ENABLED",
                ADMIN_TELEGRAM_ID,
            )
        else:
            log.error(
