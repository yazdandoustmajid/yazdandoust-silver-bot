...
async def process_manual_price_commands(client, state):
    ...
    # but not acted on.
    #
    # NOTE: this workflow runs on a 5-minute cron (price-bot.yml) with
    # concurrency.cancel-in-progress=false, and a single run can itself take
    # 4-5 minutes. That means a button press can easily sit unprocessed for
    # several minutes before the next run picks it up. The old 120s window
    # was shorter than that gap, so real admin button presses were being
    # silently dropped as "stale" (visible in the logs as a burst of
    # MANUAL PRICE UPDATE DROPPED (STALE) lines). 1800s (30 minutes) still
    # protects against replaying a menu press from hours/days ago, but no
    # longer discards a press just because the workflow run was briefly
    # delayed or queued.
    MANUAL_PRICE_UPDATE_MAX_AGE_SECONDS = 1800
...
async def send_rate_post(
    ...
    """
    if not BOT_TOKEN:
        raise RuntimeError("PRICE_BOT_TOKEN is not configured")
...
LIVE_PRICE_MONITOR_SECONDS = int(
    os.getenv("LIVE_PRICE_MONITOR_SECONDS", "280") or 280
)
