...
# Sales bot uses BOT_TOKEN; the separate price bot uses PRICE_BOT_TOKEN.
# Keeping the selection here makes the two deployments independent.
BOT_TOKEN = (
    os.getenv("PRICE_BOT_TOKEN", "")
    if BOT_ROLE == "price"
...
# Sales/admin account.  ADMIN_TELEGRAM_ID is the primary secure setting.
# The aliases make the bot tolerant of older workflow secret names.
ADMIN_TELEGRAM_ID = (
    os.getenv("ADMIN_TELEGRAM_ID")
    or os.getenv("SALES_ADMIN_TELEGRAM_ID")
    or os.getenv("TELEGRAM_ADMIN_ID")
    or ""
).strip()

# Optional username fallback.  Useful if the numeric ID secret was renamed,
# while the numeric ID remains the preferred/secure authentication method.
ADMIN_TELEGRAM_USERNAME = (
    os.getenv("ADMIN_TELEGRAM_USERNAME")
    or os.getenv("SALES_ADMIN_TELEGRAM_USERNAME")
    or ""
).strip().lstrip("@").lower()
...
def sales_customer_handle(state,message):
    ...
    # Normalize Telegram reply-keyboard text for reliable Persian matching.
    # ZWNJ/half-space, Arabic/Persian character variants and whitespace can
    # otherwise make a tapped button fail an exact string comparison.
    action_text=normalize_fa(text)
    ...
    if photos or receipt_document:
        ...
        return True
    if action_text == normalize_fa("🛒 خرید ساچمه") or (action_text and action_text.split()[0].split("@")[0].lower() == "/start"):
        c["stage"]="weight"
        save_state(state)
        sent=sales_send(cid,"⚖️ وزن مورد نظر را انتخاب کنید:",sales_weight_keyboard())
        log.info("SALES MENU ACTION | action=buy | uid=%s | send=%s",uid,sent)
        return True
    if action_text==normalize_fa("💰 قیمت لحظه‌ای"):
        p=sales_unit_price(state)
        sent=sales_send(cid,f"💰 قیمت فعلی ساچمه ۹۹۵:\n\n{sales_money(p or 0)} / گرم\n\n🔒 قیمت سفارش {SALES_PRICE_LOCK_MINUTES} دقیقه قفل می‌شود.",sales_keyboard())
        log.info("SALES MENU ACTION | action=live_price | uid=%s | send=%s | unit_price=%s",uid,sent,p)
        return True
    if action_text==normalize_fa("🧮 مقایسه قیمت"): sales_send(cid,sales_compare(state),sales_keyboard()); return True
    if action_text==normalize_fa("📦 سفارش‌های من"):
    ...
    if action_text==normalize_fa("👤 حساب من"):
    ...
    if action_text==normalize_fa("📞 پشتیبانی"): sales_send(cid,f"📞 پشتیبانی\n\n{PHONE}",sales_keyboard()); return True
    if action_text==normalize_fa("❌ لغو"):
    ...
    if active:
        ...
        if active["status"]=="awaiting_payment":
            if action_text==normalize_fa("🎟 کد تخفیف"):
            ...
            if action_text==normalize_fa("💳 پرداخت کارت"):
            ...
            if action_text==normalize_fa("🏦 پرداخت شبا"):
            ...
            if action_text==normalize_fa("🌐 پرداخت آنلاین"):
            ...
            if action_text==normalize_fa("📸 ارسال رسید"): active["status"]="awaiting_receipt"; save_state(state); sales_send(cid,"📸 عکس واضح رسید را ارسال کنید."); return True
        if active["status"]=="awaiting_receipt": sales_send(cid,"📸 لطفاً عکس رسید را ارسال کنید."); return True
    w=sales_weight(action_text)
    ...
    if action_text==normalize_fa("✏️ وزن دلخواه"): c["stage"]="weight_custom"; save_state(state); sales_send(cid,"✏️ وزن را بین ۱۰ تا ۱۰۰۰ گرم وارد کنید:",[ ["❌ لغو"] ]); return True
...
async def process_manual_price_commands(client, state):
    ...
    if not ADMIN_TELEGRAM_ID and not ADMIN_TELEGRAM_USERNAME:
        log.error(
            "PRICE ADMIN MENU DISABLED | ADMIN_TELEGRAM_ID/ADMIN_TELEGRAM_USERNAME "
            "is missing. Add ADMIN_TELEGRAM_ID to GitHub Actions Secrets."
        )
        return
    ...
    try:
        admin_id = int(ADMIN_TELEGRAM_ID) if ADMIN_TELEGRAM_ID else 0
    except Exception:
    ...
    # IMPORTANT: a manual admin command is itself the trigger for a board
    # update. Previously the override was only saved to state and the board
    # waited for an unrelated public-source price change before publishing.
    # Publish once after the whole update batch so several manual edits in the
    # same poll produce one final, consistent board.
    if changed:
        ...
        if published_message_id is not None:
            try:
                await client.send_message(
                    admin_id,
                    "✅ تابلو بلافاصله با قیمت دستی جدید به‌روزرسانی شد.",
                )
            except Exception as error:
                log.warning(
            ...
            try:
                await client.send_message(
                    admin_id,
                    "⚠️ قیمت دستی ذخیره شد، اما انتشار تابلو انجام نشد. "
    ...
    log.info(
        "MANUAL PRICE UPDATE CHECK COMPLETED | processed=%s | changed=%s",
        len(updates),
        changed,
    )
...
def install_admin_event_handler(client, state, target):
    ...
    if _ADMIN_EVENT_HANDLER_INSTALLED or not ADMIN_TELEGRAM_ID:
        return
    ...
    async def _admin_event_handler(event):
        try:
            ...
            try:
                ...
                if command in ("/pricehelp", "/manualhelp"):
                    await client.send_message(admin_id, manual_price_help_text())
                    return
...
async def run_sales_only_bot(state, duration_seconds=None):
    if duration_seconds is None:
        try:
            duration_seconds = max(60, int(os.getenv("SALES_BOT_DURATION_SECONDS", "270")))
        except (TypeError, ValueError):
    ...
    while time.monotonic() - started < duration_seconds:
        ...
        for update in updates:
            ...
            is_admin = False
            try:
                is_admin = bool(ADMIN_TELEGRAM_ID) and sender_id == int(ADMIN_TELEGRAM_ID) and chat_id == int(ADMIN_TELEGRAM_ID)
            except Exception:
                is_admin = False
            if (
                not is_admin
            ...
            try:
                ...
                if reply is not None and ADMIN_TELEGRAM_ID:
                    telegram_bot_send_message(
                        ...
                        reply,
                        reply_keyboard=sales_admin_keyboard()
                        if raw in {"🛠 مدیریت فروش", "🛠 راهنمای مدیریت", "/start", "/menu", "/sales_help"}
                        else None,
                    )
                    changed = True
            except Exception as error:
...
async def _main_locked():
    ...
    # Sales deployment uses Telegram Bot API getUpdates only; Telethon
    # API_ID/API_HASH are required only by the price-bot deployment.
    if BOT_ROLE == "price" and not API_ID:
        missing.append("API_ID")

    if BOT_ROLE == "price" and not API_HASH:
    ...
    # The sales deployment is intentionally isolated from the channel/price
