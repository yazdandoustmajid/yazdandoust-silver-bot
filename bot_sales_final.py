...
async def remove_legacy_hourly_market_reports(
    client,
    target,
    state
):
    """Permanently retire the old ``📊 گزارش لحظه‌ای بازار`` posts.

    The hourly text-only market report is disabled. On every startup we
    scan the recent channel history and delete any matching legacy posts,
    so a previously saved cleanup flag can never prevent removal of a
    report that was posted by an older process/version.
    """
    deleted = 0
    try:
        async for message in client.iter_messages(target, limit=500):
            message_text = (
                getattr(message, "raw_text", None)
                or getattr(message, "text", None)
                or ""
            )
            if "📊 گزارش لحظه‌ای بازار" not in message_text:
                continue

            try:
                await client.delete_messages(target, [int(message.id)])
                deleted += 1
            except Exception as error:
                log.warning(
                    "LEGACY HOURLY MARKET REPORT DELETE FAILED | id=%s | %s",
                    message.id,
                    error,
                )

        log.info(
            "LEGACY HOURLY MARKET REPORT CLEANUP COMPLETED | deleted=%s",
            deleted,
        )
    except Exception as error:
        log.warning(
            "LEGACY HOURLY MARKET REPORT CLEANUP FAILED: %s",
            error,
        )
...
def brand_news_media(
    media_path,
    is_video
):
    """
    Add the official logo and Telegram handle to news media.
    For videos, branding requires ffmpeg; if unavailable, the caller
    falls back to a text-only post instead of publishing unbranded video.
    """
    media_path = Path(media_path)

    if not YAZDANDOUST_LOGO.exists():
        raise RuntimeError("yazdandoust_logo.png is missing")

    handle = CHANNEL_LINK.replace(
        "https://t.me/",
        "@"
    )
    ...
    if not is_video:
        ...
        output = media_path.with_name(
            media_path.stem + "_branded.jpg"
        )
        image.save(
            output,
            "JPEG",
    ...
    if not ffmpeg:
        raise RuntimeError(
            "ffmpeg is not installed; refusing to publish unbranded video"
        )

    output = media_path.with_name(
        media_path.stem + "_branded.mp4"
    )

    # Use a temporary logo beside the media so ffmpeg can read it.
    logo_path = media_path.with_name(
        media_path.stem + "_logo.png"
    )
    Image.open(
        YAZDANDOUST_LOGO
    ).convert("RGBA").save(
    ...
    fontfile = next(
        (
            item for item in font_candidates
            if Path(item).exists()
        ),
        None
    )
    ...
    if fontfile:
        ...
        drawtext = (
            ...
            f":text='{safe_handle}'"
            ":fontsize=28:fontcolor=white@0.95"
            ":box=1:boxcolor=black@0.58:boxborderw=10"
            ":x=w-tw-28:y=h-th-28"
        )
...
async def download_news_media(
    ...
    try:
        branded_path = await asyncio.to_thread(
            brand_news_media,
            path,
            is_video
        )
        if branded_path and branded_path != path:
            path.unlink(missing_ok=True)
            path = branded_path
    except Exception as error:
        log.warning(
            "NEWS MEDIA BRANDING FAILED: %s",
            error
        )
        path.unlink(missing_ok=True)
        return None, None
...
async def send_news_post(
    ...
    # Prefer a useful source photo/video when available; otherwise use text.
    try:
        media_path, is_video = await download_news_media(article)
    except Exception:
        media_path, is_video = None, None

    if media_path is not None:
        media_caption = build_short_news_caption(article)
        if CHANNEL_LINK:
            media_caption = (
                media_caption[:NEWS_MEDIA_CAPTION_LIMIT - 40]
                + "\n\n"
                + CHANNEL_LINK.replace("https://t.me/", "@")
            )
        try:
            news_message_id = await send_news_media_post(
                client,
                target,
                media_path,
                media_caption[:NEWS_MEDIA_CAPTION_LIMIT],
                bool(is_video)
            )
            return news_message_id, media_path
        except Exception as error:
            log.warning(
                "NEWS MEDIA SEND FAILED -> TEXT FALLBACK: %s",
                error
            )
...
async def _main_locked():
    ...
    try:
        ...
        # Remove any legacy hourly text reports from the channel once.
        await remove_legacy_hourly_market_reports(
            client,
            target,
            state,
        )
        ...
        # HOURLY PRICE BOARD
        # =================================================
        # The old text-only "📊 گزارش لحظه‌ای بازار" post is retired.
        # Publish the normal price board once per hour during trading hours,
        # even when the source channel has not changed its price.

        hourly_price_window_start = (
            START_TRADES_HOUR * 60
            + START_TRADES_MINUTE
        )

        hourly_price_window_end = (
            END_TRADES_HOUR * 60
            + END_TRADES_MINUTE
        )

        hourly_price_current_minute = current_minutes()

        if (
            not is_market_holiday()
            and hourly_price_window_start
            <= hourly_price_current_minute
            < hourly_price_window_end
            and rate is not None
            and products is not None
            and should_send_hourly(state, "price_board")
        ):
            try:
                hourly_board_message_id = await publish_price_transaction(
                    client,
                    target,
                    state,
                    rate,
                ...
                if hourly_board_message_id is not None:
                    mark_hourly_sent(state, "price_board")
                    save_state(state)
                    log.info(
                        "HOURLY PRICE BOARD SENT | message_id=%s",
                        hourly_board_message_id,
                    )
                else:
                    log.warning(
                        "HOURLY PRICE BOARD NOT COMMITTED | will retry next cycle"
                    )
            except Exception as error:
                log.exception(
                    "HOURLY PRICE BOARD FAILED: %s",
                    error,
                )
