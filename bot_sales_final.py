...
async def remove_legacy_hourly_market_reports(
    ...
    """Delete old text-only hourly market reports once.

    The old ``📊 گزارش لحظه‌ای بازار`` scheduler is retired. Any matching
    messages already in the channel are removed on the first run of this
    version and the cleanup is then remembered in persistent state.
    """
    cleanup_key = "legacy_hourly_market_report_cleanup_v1"
    if state.get(cleanup_key) is True:
        return
