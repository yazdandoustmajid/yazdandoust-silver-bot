...
def create_board(
    ...
    for box, value in zip(
        ...
        # Telegram displays the 1086px board at a much smaller width in the
        # channel.  The old 25-46px range therefore made the manually entered
        # numbers look tiny.  Push the ceiling well above what the box
        # actually needs for a typical 6-7 digit toman price, so
        # fit_font_to_box's shrink loop -- not this starting point -- is
        # what decides the final size. Only very long values (>=10 chars,
        # e.g. mithqal at 2,147,500) start from a smaller ceiling.
        if len(value) >= 10:
...
def make_caption():
    ...
    return (
        ...
        "📦 معاملات بالای ۲ کیلو مشمول نرخ ویژه است، از دکمه پشتیبانی زیر با ما در تماس باشید.\n"
                            ...
                            [
                                {
                                    ...
                                    "url": f"https://t.me/{TELEGRAM_ID.lstrip('@')}",
                                },
                            ],
                        ]
                    }),
