...
async def disable_post_comments(
    ...
    forwarded channel-post id, by reading the group's recent message
    history directly. GetDiscussionMessageRequest is NOT used here: Telegram
    rejects that specific method for bot accounts (BotMethodInvalidError,
    "The API access for bot users is restricted"), even though this same
    bot is otherwise allowed to read message history in a group it's a
    member of via get_messages().
    """
    if not message_id:
        return False
    ...
    for attempt in range(1, 9):
        try:
            ...
            # IMPORTANT: linked_chat_id above is a bare integer with no
            # access_hash. Telethon cannot resolve a raw int into an entity
            # unless it happens to already be cached in the session, so
            # passing group_id directly to delete_messages() silently fails
            # with "Could not find the input entity" (caught below and
            # logged, which is why comments kept appearing despite this
            # function "succeeding" on the outside). GetFullChannelRequest
            # already returns the linked group's full Channel object (with
            # its access_hash) inside full.chats — use that instead.
            group_entity = None
            ...
            discussion = await client.get_messages(
                group_entity,
                limit=30,
            )

            discussion_messages = list(discussion or [])
