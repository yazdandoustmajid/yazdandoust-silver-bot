...
# Public price-board cadence: first valid Taqizadegan/source rate observed
# each working day, then every 90 minutes. New source rates do not trigger
# an extra immediate post; the next scheduled board uses the latest rate.
...
async def monitor_live_price_changes(
    ...
    while (
        ...
        try:
            ...
            # Automatic live-change publication is disabled. Public boards are
            # published only by the 90-minute scheduler below.
            source_changed = False
...
async def _main_locked():
    ...
    try:
        ...
        # day starts the board clock. After that, publish every 90 minutes.
        # A new source-rate message does NOT cause an immediate extra post.
        ...
        if (
            not is_market_holiday()
            and board_start_minute <= board_current_minute < board_end_minute
