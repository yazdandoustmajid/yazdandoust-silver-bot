...
async def disable_post_comments(
    ...
    """Disable comments for one channel post only.

    Telegram implements channel comments through the automatically forwarded
    channel post in the linked discussion group. Removing that starter message
    removes the comment section for this specific post.
    """
    if not message_id:
        return False
        ...
        try:
            ...
            if not discussion_messages:
                if attempt < 4:
                    ...
                    continue
                log.warning(
                    "COMMENTS DISABLE SKIPPED | no discussion starter found | post=%s",
                    message_id,
                )
                return False

            # Telegram returns these in reverse chronological order; the last
            # message is the auto-forwarded channel post that starts the thread.
            starter = discussion_messages[-1]

            group_entity = None
            starter_peer = getattr(starter, "peer_id", None)

            for chat in list(getattr(discussion, "chats", None) or []):
                peer_channel_id = getattr(starter_peer, "channel_id", None)
                if peer_channel_id is not None and getattr(chat, "id", None) == peer_channel_id:
                    group_entity = chat
                    break

            if group_entity is None:
                chats = list(getattr(discussion, "chats", None) or [])
                if chats:
                    group_entity = chats[0]
            ...
            if group_entity is None:
                if attempt < 4:
                    ...
                    continue
                log.warning(
                    "COMMENTS DISABLE FAILED | discussion group not resolved | post=%s",
                    message_id,
                )
                return False
            ...
            log.info(
                "COMMENTS DISABLED | channel_post=%s | discussion_message=%s",
                message_id,
                starter.id,
            )
            return True
    ...
    try:
        ...
        async def _disable_comments_on_any_channel_post(event):
            try:
                ...
                log.info(
                    "CHANNEL POST COMMENTS CHECKED | post=%s",
                    post_id,
                )
            except Exception as error:
                log.warning(
