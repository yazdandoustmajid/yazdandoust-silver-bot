...
# Every channel message_id that was published through one of this program's
# own publisher helpers (send_text_post / send_trade_banner / send_rate_post
# / send_market_poll / send_news_poll) this run. The CHANNEL POST COMMENTS
# GUARD below uses this to tell "a post this bot just made, whose comment
# state is already correctly handled by the helper's own allow_comments
# logic" apart from "a post someone typed manually straight into Telegram",
# which is the only case the guard is meant to touch. Without this, the
# guard would also strip comments from the bot's own analysis/poll posts,
# since Telegram bots receive channel_post updates for every post in a
# channel they belong to, including their own.
BOT_PUBLISHED_MESSAGE_IDS = set()

# Maps channel post_id -> its auto-forwarded discussion-starter message_id
# in the linked discussion group. Bot accounts are NOT allowed to call
# GetHistoryRequest/GetDiscussionMessageRequest over MTProto at all (this is
# a hard Telegram restriction with no permission that unlocks it -- unlike
# most other restrictions, admin rights do not help here). So instead of
# ever reading history to *find* the starter message, a live event listener
# (registered once, see ensure_discussion_group_listener) records each
# starter's id the moment Telegram forwards it into the group. Disabling
# comments then only ever *deletes* a known message_id, which bots are
# allowed to do.
DISCUSSION_STARTER_CACHE = {}
DISCUSSION_GROUP_LISTENER_REGISTERED = False
...
async def send_news_poll(
    ...
    try:
        ...
        BOT_PUBLISHED_MESSAGE_IDS.add(message_id)
...
def sales_admin_handle(state,raw):
    # Sales management belongs exclusively to the dedicated sales bot.
    if BOT_ROLE == "price":
        return None
    parts=raw.split()
...
async def process_manual_price_commands(client, target, state):
    ...
    params = {
        "limit": 100,
        "timeout": 5,
        # We also consume channel_post updates so the price bot can
        # immediately remove Telegram's discussion starter from ANY post
        # made manually in the public channel.
    ...
    for update in updates:
        ...
        # ---------------------------------------------------------
        # CHANNEL POST COMMENTS GUARD
        # ---------------------------------------------------------
        # Manual posts arrive as `channel_post`, not `message`.  When the
        # channel has a linked discussion group, Telegram creates a
        # discussion starter and shows the Comments button.  Remove that
        # starter immediately so manual posts stay comment-free.
        channel_post = update.get("channel_post") or {}
        if channel_post:
            try:
                post_id = int(channel_post.get("message_id", 0) or 0)
                if post_id and post_id in BOT_PUBLISHED_MESSAGE_IDS:
                    # This post was published by one of our own send_*
                    # helpers, which already applied the correct
                    # allow_comments decision for it (or is an intentional
                    # poll). Do not touch it again here, or analysis/poll
                    # posts that are meant to keep comments would lose them.
                    BOT_PUBLISHED_MESSAGE_IDS.discard(post_id)
                    log.info(
                        "CHANNEL POST ALREADY HANDLED BY PUBLISHER | post=%s | skipped",
                        post_id,
                    )
                elif post_id:
                    log.info(
                        "CHANNEL POST DETECTED | post=%s | disabling comments",
                        post_id,
                    )
                    disabled = await disable_post_comments(
                        client,
                        target,
                        post_id,
                    )
                    if disabled:
                        log.info(
                            "CHANNEL POST COMMENTS DISABLED | post=%s",
                            post_id,
                        )
                    else:
                        log.warning(
                            "CHANNEL POST COMMENTS DISABLE FAILED | post=%s",
                            post_id,
                        )
                else:
                    log.warning(
                        "CHANNEL POST WITHOUT MESSAGE_ID | update_id=%s",
                        update_id,
                    )
            except Exception as error:
                log.exception(
                    "CHANNEL POST COMMENTS HANDLER FAILED | post=%s | error=%s",
        ...
        for lang in languages:
            for psm in (6, 11, 12):
                ...
                for tokens in lines.values():
                    ...
                    # Bullion price must specifically be the imported
                    # "نادیر/ندیر" (Nadir) bar row. TGH's table also lists
                    # its own "شمش تخصصی TGH" bullion and a plain 999.9
                    # row -- a generic "شمش"/"999" match used to grab
                    # whichever row OCR happened to read, sometimes the
                    # wrong product entirely.
                    has_bullion = (
                        "نادیر" in normalized
                        or "ندیر" in normalized
                    )
...
TAGHIZADEGAN_NADIR_1KG_URL = os.getenv(
    "TAGHIZADEGAN_NADIR_1KG_URL",
    "https://taghizadegan.com/product/silver-nadir-1k/"
).strip()


def get_taghizadegan_nadir_price_per_gram():
    """Fetch the Nadir 1kg silver bar's regular (pre-discount) price from
    taghizadegan.com's WooCommerce product page and convert it to a
    per-gram toman price.

    This is the second-priority automatic source for the نادیر/ندیر 999.9
    bullion price (after the TGH channel table OCR). Returns None on any
    ...
    try:
        ...
        match = re.search(
            r"\u0642\u06cc\u0645\u062a\s*\u0627\u0635\u0644\u06cc[:\s]*([\d\u06f0-\u06f9,.\u066c]+)\s*\u062a\u0648\u0645\u0627\u0646",
            html,
        )
        if not match:
            log.warning("TAGHIZADEGAN NADIR PRICE NOT FOUND IN PAGE")
            return None

        raw_number = normalize_digits(match.group(1))
        digits_only = re.sub(r"[^\d]", "", raw_number)
        if not digits_only:
            return None

        total_price_1kg = int(digits_only)
        per_gram = total_price_1kg // 1000

        if not (100_000 <= per_gram <= 2_000_000):
            log.warning(
                "TAGHIZADEGAN NADIR PRICE OUT OF RANGE | total_1kg=%s | per_gram=%s",
                total_price_1kg, per_gram,
            )
            return None

        log.info(
            "TAGHIZADEGAN NADIR PRICE FOUND | total_1kg=%s | per_gram=%s",
            total_price_1kg, per_gram,
        )
        return int(per_gram)

    except Exception as error:
        log.warning("TAGHIZADEGAN NADIR PRICE FETCH FAILED | %s", error)
        return None


_LAST_KNOWN_TGH_BULLION_PRICE = None
_LAST_KNOWN_TGH_BULLION_PRICE_LOADED = False
...
def find_latest_tgh_product_prices():
    ...
    fallback here.
    """
    global _LAST_KNOWN_TGH_BULLION_PRICE, _LAST_KNOWN_TGH_BULLION_PRICE_LOADED

    if not _LAST_KNOWN_TGH_BULLION_PRICE_LOADED:
        try:
            _saved_state = load_state()
            if _saved_state.get("nader_9999") is not None:
                _LAST_KNOWN_TGH_BULLION_PRICE = int(_saved_state["nader_9999"])
        except Exception:
            pass
        _LAST_KNOWN_TGH_BULLION_PRICE_LOADED = True
    ...
    for page_number in range(1, 8):
        ...
        for message in messages:
            ...
            try:
                ...
                if bullion_price is not None:
                    _LAST_KNOWN_TGH_BULLION_PRICE = bullion_price

                bullion_source_note = ""
                effective_bullion = bullion_price

                if effective_bullion is None:
                    website_bullion = get_taghizadegan_nadir_price_per_gram()
                    if website_bullion is not None:
                        effective_bullion = website_bullion
                        _LAST_KNOWN_TGH_BULLION_PRICE = website_bullion
                        bullion_source_note = " (from taghizadegan.com, channel OCR miss)"

                if effective_bullion is None:
                    effective_bullion = _LAST_KNOWN_TGH_BULLION_PRICE
                    if effective_bullion is not None:
                        bullion_source_note = " (carried over, OCR and website both missed)"

                if shot_price is not None and effective_bullion is not None:
                    log.info(
                        "TGH PRODUCT PRICES FOUND | shot_995=%s | bullion_9999=%s | message=%s%s",
                        shot_price,
                        effective_bullion,
                        message["message_id"],
                        bullion_source_note,
                    )
                    return int(shot_price), int(effective_bullion), int(message["message_id"])
...
async def send_market_poll(
    ...
    BOT_PUBLISHED_MESSAGE_IDS.add(int(sent.id))
...
async def ensure_discussion_group_listener(client, target):
    """Register a live listener that records each discussion-starter's id
    the moment Telegram auto-forwards a channel post into the linked
    discussion group.

    This exists because bot accounts cannot call
    GetHistoryRequest/GetDiscussionMessageRequest over MTProto at all --
    Telegram rejects both with BotMethodInvalidError regardless of admin
    rights. Receiving live updates is allowed for bots, so this is the only
    way a bot can ever learn which discussion-group message corresponds to
    which channel post. Safe to call repeatedly; only registers once.
    """
    global DISCUSSION_GROUP_LISTENER_REGISTERED

    if DISCUSSION_GROUP_LISTENER_REGISTERED:
        return True
    ...
    try:
        ...
        if not group_id:
            log.warning(
                "DISCUSSION GROUP LISTENER NOT REGISTERED | no linked discussion group"
            )
            return False

        group_entity = None
        for chat in getattr(full, "chats", None) or []:
            if int(getattr(chat, "id", 0) or 0) == int(group_id):
                group_entity = chat
                break

        if group_entity is None:
            log.warning(
                "DISCUSSION GROUP LISTENER NOT REGISTERED | could not resolve "
                "discussion group entity (id=%s)",
                group_id,
            )
            return False

        @client.on(events.NewMessage(chats=group_entity))
        async def _record_discussion_starter(event):
            try:
                fwd = getattr(event.message, "fwd_from", None)
                channel_post = getattr(fwd, "channel_post", None)
                if channel_post is not None:
                    DISCUSSION_STARTER_CACHE[int(channel_post)] = int(event.message.id)
                    log.info(
                        "DISCUSSION STARTER RECORDED | channel_post=%s | discussion_message=%s",
                        channel_post,
                        event.message.id,
                    )
            except Exception as error:
                log.warning(
                    "DISCUSSION STARTER RECORD FAILED | %s",
                    error,
                )

        DISCUSSION_GROUP_LISTENER_REGISTERED = True
        log.info(
            "DISCUSSION GROUP LISTENER REGISTERED | discussion_group=%s",
            group_id,
        )
        return True
        ...
        log.warning(
            "DISCUSSION GROUP LISTENER REGISTRATION FAILED | %s",
            error,
        )
        return False
...
async def disable_post_comments(
    ...
    Bot accounts cannot call GetHistoryRequest/GetDiscussionMessageRequest
    over MTProto at all (BotMethodInvalidError, "The API access for bot
    users is restricted") -- this is unconditional and admin rights do not
    unlock it. So this function never reads the group's history to *find*
    the starter message; it only ever *deletes* one, by id, once
    ensure_discussion_group_listener has recorded it in
    DISCUSSION_STARTER_CACHE from a live update. delete_messages() is a
    normal write action a bot admin is allowed to perform.
    """
    if not message_id:
        return False
    ...
    try:
        ...
        if group_entity is None:
            log.warning(
                "COMMENTS DISABLE FAILED | could not resolve discussion "
                "group entity (id=%s) from GetFullChannelRequest.chats | "
                "post=%s",
                group_id,
                message_id,
            )
        ...
        log.warning(
            "COMMENTS DISABLE FAILED | could not resolve channel/group | post=%s | error=%s",
            message_id,
            error,
        )
        return False

    await ensure_discussion_group_listener(client, target)

    starter_message_id = None

    for attempt in range(1, 9):
        starter_message_id = DISCUSSION_STARTER_CACHE.pop(int(message_id), None)
    ...
    if starter_message_id is None:
        log.warning(
            "COMMENTS DISABLE FAILED | discussion starter never recorded by listener | "
            "post=%s | group=%s",
            message_id,
            group_id,
        )
...
async def monitor_live_price_changes(
    ...
    while (
        ...
        # ---------------------------------------------------------
        # START TRADES AT EXACTLY 10:00
        # ---------------------------------------------------------
        # The outer scheduler can start a few minutes before 10:00 and the
        # live monitor may cross the 10:00 boundary while still running.
        # Check here as well so the opening-trades banner is not delayed
        # until the next scheduler cycle.
        if (
            BOT_ROLE == "price"
            and not is_market_holiday()
            and current_minutes() >= START_TRADES_HOUR * 60 + START_TRADES_MINUTE
            and current_minutes() < END_TRADES_HOUR * 60
            and should_send_daily(state, "start_trades")
        ):
            try:
                ...
                if message_id:
                    mark_daily_sent(state, "start_trades")
                    save_state(state)
                    log.info("START TRADES BANNER SENT BY LIVE MONITOR | message_id=%s", message_id)
            except Exception as error:
                log.exception("START TRADES LIVE BANNER FAILED: %s", error)
        ...
        try:
            ...
            # Channel publication is driven by a new Taqizadegan source
            # message OR a real change in the two core products (995 shot and
            # 999.9 ingot). A new source message is enough to publish
            # immediately, so the board never waits for the 90-minute clock.
            ...
            if not isinstance(products_probe, dict):
                ...
                # PRIMARY LIVE TRIGGER: use the exact Taqizadegan/TGH product-table
                # message ID returned by the OCR parser. A new board can be an
                # image-only post with no caption containing both ounce and dollar,
                # so the older text-rate message ID is not reliable enough.
                tgh_product_source_id = (
                    products_probe.get("shot_source_message_id")
                    or products_probe.get("nader_source_message_id")
                )
                try:
                    tgh_product_source_id = (
                        int(tgh_product_source_id)
                        if tgh_product_source_id is not None
                        else None
                    )
                except (TypeError, ValueError):
                    tgh_product_source_id = None

                previous_tgh_product_source_id = state.get(
                    "tgh_product_source_message_id"
                )
                try:
                    previous_tgh_product_source_id = (
                        int(previous_tgh_product_source_id)
                        if previous_tgh_product_source_id is not None
                        else None
                    )
                except (TypeError, ValueError):
                    previous_tgh_product_source_id = None

                tgh_product_message_changed = (
                    tgh_product_source_id is not None
                    and tgh_product_source_id != previous_tgh_product_source_id
                )

                # Keep the old text-rate trigger as a secondary fallback.
                ...
                # A new Taqizadegan product-table post OR a real product-price
                # change must publish immediately; do not wait for the 90-minute
                # board clock.
                source_changed = (
                    product_changed
                    or source_message_changed
                    or tgh_product_message_changed
                )
                        ...
                        if current_signature != previous_signature:
                            if validate_price_update(
                                ...
                                if immediate_message_id is not None:
                                    ...
                                    if tgh_product_source_id is not None:
                                        state["tgh_product_source_message_id"] = int(
                                            tgh_product_source_id
                                        )
...
async def _main_locked():
    ...
    try:
        ...
        # Register the discussion-group starter listener as early as
        # possible, before any posts go out this cycle. See
        # ensure_discussion_group_listener() for why this exists: bots
        # can't read chat history at all, so this is the only way to learn
        # which discussion-group message belongs to which channel post.
        await ensure_discussion_group_listener(client, target)

        # Manual channel-post comment control is handled by the Bot API
        # `channel_post` consumer in process_manual_price_commands().
        # Do not register a second Telethon handler; that would duplicate
        # delete attempts and create misleading logs.
