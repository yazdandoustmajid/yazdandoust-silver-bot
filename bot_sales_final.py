...
async def process_manual_price_commands(client, state):
    ...
    for update in updates:
        ...
        if message_age > MANUAL_PRICE_UPDATE_MAX_AGE_SECONDS:
            log.info(
                "MANUAL PRICE UPDATE DROPPED (STALE) | update_id=%s | age=%.0fs",
                update_id,
                message_age,
            )
            changed = True
...
LIVE_PRICE_MONITOR_SECONDS = int(
    os.getenv("LIVE_PRICE_MONITOR_SECONDS", "210") or 210
)
...
LIVE_MIN_TEHRAN_CHANGE_RIAL = int(
    os.getenv("LIVE_MIN_TEHRAN_CHANGE_RIAL", "3000") or 3000
)
LIVE_MIN_OUNCE_CHANGE_USD = float(
    os.getenv("LIVE_MIN_OUNCE_CHANGE_USD", "0.5") or 0.5
)
LIVE_POST_MIN_INTERVAL_SECONDS = int(
    os.getenv("LIVE_POST_MIN_INTERVAL_SECONDS", "90") or 90
)
...
async def _main_locked():
    ...
    # =========================================================
    # Older deployments could leave manual_price_overrides in state.json.
    ...
    # commands (/price and the price menu) still work exactly as before.
    # The migration runs once, so a new manual price entered later is not
    # wiped on every GitHub Actions execution.
    if BOT_ROLE == "price" and not state.get("manual_price_auto_mode_migrated_v2"):
        ...
        state.pop(PRICE_MENU_DRAFT_KEY, None)
        state.pop("price_menu_pending_publish", None)
        state["manual_price_auto_mode_migrated_v2"] = True
        save_state(state)

        if stale_manual_state:
            log.warning(
                "MANUAL PRICE MIGRATION V2 | stale manual overrides cleared | "
                "AUTOMATIC LIVE PRICE MODE RESTORED"
            )
        else:
            log.info(
                "MANUAL PRICE MIGRATION V2 | no stale manual overrides found"
            )
